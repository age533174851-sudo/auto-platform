...
### 호가창 — 해결됨 (원인: 심볼 슬래시)
`sel.sym`이 'BTC/USDT' 형태라 `.replace(/USDT$/,'') + 'USDT'`가 'BTC/USDT'를 만들었고,
`btc/usdt@depth20@100ms`라는 없는 스트림에 붙었다. Binance는 연결만 받아주고 데이터를
보내지 않아서 "● 실시간인데 호가가 비어 있는" 상태가 됐다.
→ 영숫자만 남기도록 수정. 브라우저에서 실제 호가 수신 확인 완료.
...
확인할 것:
- 브라우저 콘솔에서 해당 스트림의 프레임이 실제로 오는지 (depth만 오는지)
- 결합 스트림 URL의 `@aggTrade` 대소문자
- onmessage의 분기 순서(현재 depth → aggTrade → ticker, 각각 return)

### (구) WebSocket 호가창 — 코드 완료, 시각 확인 미완
브라우저 확인 결과 — **WebSocket 연결은 성공하지만(`● 실시간` 표시) 호가 데이터가 오지 않는다.**
화면에 "호가 수신 대기 중…"만 계속 보인다.

재현: 로컬 `npm run dev` → 매매 → 상단 서브탭 '매매'(y≈206, 하단 네비 아님) → BTC 선택
확인할 것:
- `wss://fstream.binance.com/stream?streams=btcusdt@depth20@100ms/...` 스트림명이 맞는지
- 결합 스트림 래퍼가 `{stream, data}`인지, `data.a`/`data.b` 필드명이 맞는지
- 브라우저 콘솔/네트워크 탭에서 WS 프레임이 실제로 오는지
- Vercel 배포본에서도 같은지 (로컬만의 문제인지 구분)
...
사용자가 꼽은 핵심 5가지:
1. **거래소 ↔ 앱 실시간 상태 대조** — 포지션·미체결·레버리지·마진타입·TP/SL·잔고.
   불일치 시 신규 주문 잠금 + 경고
