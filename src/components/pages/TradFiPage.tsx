...
import type { DataFreq, ProductType, RealtimeConfig, TradFiAsset, TradFiProvider } from '@/types/domain';
            ...
            {filtered.map((a,i)=>(
              <div key={a.id} onClick={()=>openTrade(a)} style={{display:'grid',gridTemplateColumns:'1fr auto 80px',padding:'12px 14px',borderBottom:i<filtered.length-1?`1px solid ${T.border}`:'none',alignItems:'center',cursor:a.isWatchOnly?'default':'pointer'}}>
                ...
                <div style={{textAlign:'right'}}>
                  <div style={{color:T.txt,fontWeight:700,fontSize:11,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{a.p.toLocaleString()}</div>
      ...
      {tab==='cfd'&&mode==='trade'&&sel&&(
        <div>
          ...
          <Card style={{padding:'14px 16px',marginBottom:12}}>
            ...
            <div style={{color:T.txt,fontSize:24,fontWeight:900,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',marginTop:4}}>{sel.p.toLocaleString()} {sel.category==='forex'?'':currency==='USD'?'USD':'USDT'}</div>
          ...
          <Card style={{padding:14,marginBottom:12}}>
            ...
            {/* Amount */}
            <input type="number" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="USDT 금액 입력" style={{width:'100%',background:T.alt,border:`1px solid ${T.border2}`,borderRadius:10,padding:'12px 14px',color:T.txt,fontSize:16,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',fontWeight:700,outline:'none',marginBottom:8}}/>
