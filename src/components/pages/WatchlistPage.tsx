      ...
      {cat==='관심종목'&&wl.length===0?(
        ...
        <Card style={{overflow:'hidden'}}>
          {displayRows.map((a:any,i:number)=>(
            <div key={a.id} className="wl-row" style={{display:'flex',alignItems:'center',padding:'10px 12px',borderBottom:i<displayRows.length-1?`1px solid ${T.border}`:'none',cursor:'pointer',gap:8}} onClick={()=>onNav('chart')}>
              {/* Left: logo + name */}
              ...
              <div className="wl-row-right">
                {a.p?(
                  <>
                    <div style={{color:T.txt,fontWeight:700,fontSize:11,fontFamily:'monospace',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:'100%'}}>{cvt(a.p,currency)}</div>
