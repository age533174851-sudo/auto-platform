                ...
                {(Array.isArray(linkedBanks)?linkedBanks:[]).map((b,i)=>(
                  <div key={b.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:i<linkedBanks.length-1?`1px solid ${T.border}`:'none'}}>
                    ...
                    <div style={{textAlign:'right'}}>
                      <div style={{color:T.txt,fontSize:13,fontWeight:700,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{cvt(b.balance,currency)}</div>
          ...
          {obTab==='deposit'&&(
            <Card style={{padding:'14px 16px'}}>
              ...
              <div style={{background:T.alt,borderRadius:10,padding:'14px',marginBottom:12,border:`1px solid ${T.border}`}}>
                ...
                <div style={{color:T.txt,fontWeight:900,fontSize:16,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>신한은행 110-123-456789</div>
        ...
        <div>
          <Card style={{padding:'14px 16px',marginBottom:12}}>
            ...
            <div style={{background:T.alt,borderRadius:10,padding:'12px 14px',marginBottom:10,border:`1px solid ${T.border}`}}>
              ...
              <div style={{color:T.txt,fontSize:18,fontWeight:900,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>1 {fxPair.from} = {fxPair.rate.toFixed(4)} {fxPair.to}</div>
        ...
        <>
          ...
          <div style={{position:'fixed',inset:'auto 0 0',zIndex:201,background:T.surf,borderRadius:'20px 20px 0 0',padding:'24px 20px calc(40px + env(safe-area-inset-bottom, 0px))',maxWidth:480,margin:'0 auto',border:`1px solid ${T.border}`}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',gap:10,marginTop:16}}>
              ...
              <button onClick={()=>setShowConfirm(false)} style={{flex:2,padding:'13px',minHeight:48,background:`linear-gradient(135deg,${T.acc},${T.prp})`,color:'#fff',border:'none',borderRadius:12,fontWeight:800,fontSize:13,cursor:'pointer'}}>모의 이체 확인</button>
            </div>
          </div>
        </>
      )}
