...
import type { BadgeType, InviteCode, PlanStatus, PlanType, UserSubscription } from '@/types/domain';
...
function SubscriptionPage() {
  ...
  return (
    <div>
        ...
        <div>
          ...
          <Card style={{padding:'14px 16px',marginBottom:12}}>
            ...
            {[
              ...
              {l:'초대 코드',v:<span style={{color:T.muted,fontSize:12,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{sub.inviteCode||'-'}</span>},
            ].map((r,i)=>(
          ...
          <Card style={{padding:'14px 16px',marginBottom:14}}>
            ...
            <div style={{display:'flex',gap:8,marginBottom:8}}>
              <input
                ...
                style={{flex:1,background:T.alt,border:`1px solid ${T.border2}`,borderRadius:10,padding:'12px 14px',color:T.txt,fontSize:13,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',fontWeight:700,outline:'none',letterSpacing:1}}
              />
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {MOCK_INVITE_CODES.filter(c=>c.active).map(c=>(
                <div key={c.id} onClick={()=>setRedeemCode(c.code)} style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:T.alt,borderRadius:8,padding:'8px 12px',cursor:'pointer',border:`1px solid ${T.border}`}}>
                  <div>
                    <div style={{color:T.txt,fontSize:11,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',fontWeight:700}}>{c.code}</div>
      ...
      {tab==='admin'&&(
        <div>
          <div style={{color:T.txt,fontWeight:800,fontSize:15,marginBottom:4}}>구독 관리자</div>
          {/* Admin sub-tabs */}
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10,marginBottom:14}}>
                {[
                  ...
                  <Card key={s.l} style={{padding:'14px 12px'}}>
                    ...
                    <div style={{color:s.c,fontSize:22,fontWeight:900,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{s.v}</div>
                  </Card>
                ))}
              </div>
              <Card style={{padding:'14px 16px',marginBottom:12}}>
          ...
          {grantModal&&(
            <>
              ...
              <div style={{position:'fixed',inset:'auto 0 0',zIndex:201,background:T.surf,borderRadius:'20px 20px 0 0',padding:'24px 20px calc(40px + env(safe-area-inset-bottom, 0px))',maxWidth:480,margin:'0 auto',border:`1px solid ${T.border}`}} onClick={e=>e.stopPropagation()}>
                <div style={{color:T.txt,fontWeight:800,fontSize:16,marginBottom:4}}>플랜 변경</div>
