...
import { toTradingViewSymbol as tvSymbol } from '@/lib/tvSymbol';
      ...
      <div style={{background:'linear-gradient(135deg,var(--t-bg),#0A0F1E)',border:`1px solid ${sel.clr}40`,borderRadius:18,padding:'14px 16px',marginBottom:12}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
          ...
          <div style={{textAlign:'right',flexShrink:0}}>
            {selLive?(
              <div>
                <div style={{color:T.txt,fontWeight:700,fontSize:13,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>
        ...
        <div style={{marginBottom:8}}>
          <div style={{display:'flex',gap:6,marginBottom:5}}>
            <input value={manualSym} onChange={e=>setManualSym(e.target.value.toUpperCase())} onKeyDown={e=>e.key==='Enter'&&applyManual()}
              ...
              style={{flex:1,background:T.card,border:`1px solid ${T.acl}`,borderRadius:10,padding:'9px 12px',color:T.acl,fontSize:12,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',fontWeight:700,outline:'none',letterSpacing:.5}}/>
          ...
          <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
            {['NASDAQ:AAPL','NYSE:KO','AMEX:SPY','KRX:005930','BINANCE:BTCUSDT','OANDA:XAUUSD'].map(ex=>(
              <button key={ex} onClick={()=>setManualSym(ex)} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:6,padding:'3px 8px',color:T.muted,fontSize:9,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',cursor:'pointer'}}>{ex}</button>
            ))}
          </div>
        </div>
      )}
