...
function TradingPage({prices,currency,activeAsset}:{prices:Asset[];currency:string;activeAsset?:any}) {
  ...
  const [riskProfile,setRiskProfile]=useState<'conservative'|'balanced'|'aggressive'>('balanced');
    ...
    const [tab,setTab]=useState<'trade'|'chart'|'sizing'|'risk'>('trade');
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:6,marginBottom:12}}>
        {(['trade','chart','sizing','risk'] as const).map(t=>(
          <button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:'8px',background:tab===t?T.acg:'transparent',color:tab===t?T.acl:T.muted,border:`1px solid ${tab===t?T.acl:T.border}`,borderRadius:10,fontSize:11,fontWeight:700,cursor:'pointer'}}>
            {t==='trade'?'⚡ 매매':t==='chart'?'📊 차트':t==='sizing'?'🎯 사이징':'🔥 리스크'}
          </button>
        ))}
      </div>
      ...
      {tab==='chart'&&(
        <Card style={{padding:0,overflow:'hidden',marginBottom:12}}>
          ...
          <div style={{padding:'8px 14px',borderTop:`1px solid ${T.border}`,display:'flex',gap:8}}>
            <button onClick={()=>{setSide('매수');setTab('trade');}} style={{flex:1,padding:'10px',background:T.grn,color:'#fff',border:'none',borderRadius:10,fontWeight:800,fontSize:13,cursor:'pointer',minHeight:40}}>
              📈 매수
            </button>
            <button onClick={()=>{setSide('매도');setTab('trade');}} style={{flex:1,padding:'10px',background:T.red,color:'#fff',border:'none',borderRadius:10,fontWeight:800,fontSize:13,cursor:'pointer',minHeight:40}}>
              📉 매도
            </button>
          </div>
        </Card>
