                  ...
                  <div key={i} style={{background:T.alt,borderRadius:10,padding:'11px 13px',marginBottom:i<positions.length-1?8:0}}>
                    ...
                    {(quickActions.includes('reverse')||quickActions.includes('tpsl'))&&<div style={{display:'flex',gap:6,marginTop:6}}>
                      {quickActions.includes('reverse')&&<button onClick={()=>{
                        const wasShort = p.side==='short';
                        const closedValue = p.qty * cur;   // 청산 금액
                        const res=reversePaperPosition(p.asset,cur);
                        if(res.ok){
                          // 반대 방향으로 같은 금액 재진입
                          const newSide = wasShort ? 'long' : 'short';
                          paperBuy(p.asset, cur, Math.round(closedValue), { stratId:'reverse', side:newSide });
                          refreshPositions();
                          alert(`리버스 완료!\n기존 ${wasShort?'숏':'롱'} 청산 (실현손익 ${Math.round(res.pnl).toLocaleString('ko-KR')}원)\n→ ${newSide==='long'?'롱':'숏'} 신규 진입`);
                        ...
                        <div style={{marginBottom:12}}>
                          {[['현재가',cur],['진입가',p.avgPrice],['청산가',isShort?p.avgPrice*(1+0.9/lev):p.avgPrice*(1-0.9/lev)]].map(([l,v]:any,i)=>(
                            <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'3px 0',fontSize:11}}>
                              ...
                              <span style={{color:l==='청산가'?T.ylw:T.txt,fontWeight:700,fontFamily:'monospace'}}>₩{fmt(Math.round(v))}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        {tpslTab==='trailing'?(
                          ...
                          <>
                            ...
                            <div style={{marginBottom:10}}>
                              ...
                              <div style={{display:'flex',gap:6}}>
                                <div style={{flex:1,display:'flex',alignItems:'center',background:T.alt,border:`1px solid ${T.grn}40`,borderRadius:7,padding:'0 10px'}}>
                                  <input value={tpEditVal} onChange={e=>{setTpEditVal(e.target.value.replace(/[^0-9.]/g,''));setTpRoi('');}} placeholder="목표 가격" inputMode="decimal"
                                    style={{flex:1,background:'transparent',border:'none',outline:'none',color:T.txt,fontSize:12,padding:'9px 0'}}/>
                              ...
                              {(tpPrice>0)&&<div style={{color:T.muted,fontSize:9,marginTop:4,fontFamily:'monospace'}}>→ 익절가 ₩{fmt(Math.round(tpPrice))} (ROI {priceToRoi(tpPrice).toFixed(1)}%)</div>}
                            </div>
                            {/* SL */}
                            <div style={{marginBottom:12}}>
                              <div style={{color:T.red,fontSize:10,fontWeight:700,marginBottom:4}}>손절 (SL)</div>
                              <div style={{display:'flex',gap:6}}>
                                <div style={{flex:1,display:'flex',alignItems:'center',background:T.alt,border:`1px solid ${T.red}40`,borderRadius:7,padding:'0 10px'}}>
                              ...
                              {(slPrice>0)&&<div style={{color:T.muted,fontSize:9,marginTop:4,fontFamily:'monospace'}}>→ 손절가 ₩{fmt(Math.round(slPrice))} (ROI {priceToRoi(slPrice).toFixed(1)}%)</div>}
                            </div>
                            {tpslTab==='partial'&&(
                              <div style={{marginBottom:12}}>
