                  ...
                  <div key={i} style={{background:T.alt,borderRadius:10,padding:'11px 13px',marginBottom:i<positions.length-1?8:0}}>
                    ...
                    {(quickActions.includes('reverse')||quickActions.includes('tpsl'))&&<div style={{display:'flex',gap:6,marginTop:6}}>
                      {quickActions.includes('reverse')&&<button onClick={()=>{
                        ...
                        // 1) 전량 청산
                        const res = closePaperPosition(p.asset, cur, 1);
                        // 2) 청산 직후 반대방향 재진입 (진입 금액 = 기존 명목)
                        const reAmt = Math.max(10000, Math.round(p.qty * cur));
                        const buyRes = paperBuy(p.asset, cur, reAmt, { stratId:'reverse', side:newSide });
                        refreshPositions();
                        if(buyRes.ok){
                          alert(`리버스 완료!\n기존 ${wasShort?'숏':'롱'} 청산 (실현손익 ${Math.round(res.pnl||0).toLocaleString('ko-KR')}원)\n→ ${newSide==='long'?'롱':'숏'} 신규 진입`);
                        } else {
                          alert(`청산은 됐지만 반대 진입 실패: ${buyRes.reason||'잔고 부족'}`);
                        }
