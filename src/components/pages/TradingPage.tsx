    ...
    <div>
      ...
      {tab==='trade'&&(
        <>
          ...
          {/* Order form — 바이낸스 2단 레이아웃 (좌: 주문 / 우: 오더북) */}
          <Card style={{padding:12,marginBottom:12}}>
            <div style={{display:'grid',gridTemplateColumns:'1.15fr 0.85fr',gap:10,alignItems:'start'}}>
              {/* ── 좌측: 주문 컨트롤 ── */}
              <div>
            {/* Buy/Sell */}
            <div style={{display:'flex',gap:8,marginBottom:12}}>
              ...
              <div onClick={()=>setShowLevSheet(false)} style={{position:'fixed',inset:0,zIndex:3000,background:'rgba(0,0,0,.6)',display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
                <div onClick={e=>e.stopPropagation()} style={{width:'100%',maxWidth:480,background:T.surf,borderRadius:'20px 20px 0 0',padding:'20px 18px calc(20px + env(safe-area-inset-bottom,0px))',border:`1px solid ${T.border2}`}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',background:T.alt,borderRadius:12,padding:'4px',marginBottom:18}}>
                    ...
                    <div style={{flex:1,textAlign:'center',color:leverage>20?T.red:leverage>5?T.ylw:T.grn,fontSize:28,fontWeight:900,fontFamily:'monospace'}}>{leverage}<span style={{fontSize:18}}>x</span></div>
            ...
            <button onClick={()=>{if(!amount)setAmount('100000');setShowConfirm(true);}} disabled={status==='loading'}
              style={{width:'100%',padding:'14px',background:status==='done'?T.grn:status==='loading'?'#243A5E':(side==='매수'?T.grn:T.red),color:'#fff',border:'none',borderRadius:10,fontWeight:900,fontSize:14,cursor:'pointer'}}>
              {status==='loading'?'처리 중…':status==='done'?'✅ 완료!':`[${tradeMode==='mock'?'모의':tradeMode==='testnet'?'테스트넷':'실전'}] ${side==='매수'?'매수/롱':'매도/숏'} ${leverage}x`}
            </button>
            <div style={{color:T.muted,fontSize:9,textAlign:'center',marginTop:5}}>금액 미입력 시 기본 10만원</div>
              </div>{/* 좌측 끝 */}
              ...
              <div>
                {(()=>{
                  ...
                  const asks = Array.from({length:8},(_,i)=>({ p: px+tick*(8-i), amt: (Math.random()*80+10) }));
                  const bids = Array.from({length:8},(_,i)=>({ p: px-tick*(i+1), amt: (Math.random()*80+10) }));
