  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:5,marginBottom:14,overflowX:'auto'}}>
        {([['accounts','📱 계좌 현황'],['connect','API 연결'],['bulk','일괄 매매'],['safety','안전 설정']] as const).map(([id,label])=>(
          ...
          {(['longterm','shortterm','auto','cash'] as AccountGroup[]).map(grp=>{
            ...
            return (
              <div key={grp} style={{marginBottom:14}}>
                ...
                {(Array.isArray(grpAccs)?grpAccs:[]).map((acc,i)=>{
                    ...
                    <div key={acc.id} onClick={()=>toggleAccount(acc.id)} style={{background:T.card,border:`2px solid ${isSel?T.acl:T.border}`,borderRadius:16,padding:'14px',marginBottom:8,cursor:'pointer',position:'relative'}}>
                      ...
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                        ...
                        <div style={{textAlign:'right'}}>
                          <div style={{color:T.txt,fontWeight:700,fontSize:13,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{cvt(acc.balance,currency)}</div>
            ...
            <div style={{position:'sticky',bottom:80,background:T.surf,border:`1px solid ${T.acl}40`,borderRadius:16,padding:'12px 14px',boxShadow:'0 -4px 20px rgba(0,0,0,.4)',zIndex:20}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <div style={{display:'flex',gap:8}}>
                  ...
                  <button onClick={()=>setTab('bulk')} style={{background:T.acc,color:'#fff',border:'none',borderRadius:8,padding:'8px 14px',minHeight:36,fontSize:11,fontWeight:700,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:5}}>
                    <PackageOpen size={12} strokeWidth={2.4}/>일괄 매매 →
                  </button>
                </div>
              </div>
            </div>
          ...
          {connectStep>=4&&(
            <Card style={{padding:'14px 16px',marginBottom:12}}>
              ...
              <div style={{marginBottom:10}}>
                ...
                <input value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder="API Key 입력" style={{width:'100%',background:T.alt,border:`1px solid ${T.border}`,borderRadius:8,padding:'10px 12px',color:T.txt,fontSize:12,outline:'none',fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}/>
              </div>
                ...
                return selAccObjs.map(acc=>{
                  ...
                  return (
                    <div key={acc.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:`1px solid ${T.border}`}}>
                      ...
                      <div style={{textAlign:'right'}}><div style={{color:T.txt,fontSize:12,fontWeight:700,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{cvt(amt,currency)}</div><div style={{color:T.muted,fontSize:10}}>수수료 {cvt(fee,currency)}</div></div>
                    </div>
                  );
                });
              })()}
      ...
      {tab==='safety'&&(
        <div>
          ...
          {/* Per-account safety */}
          <Card style={{padding:'14px 16px',marginBottom:12}}>
            <div style={{color:T.txt,fontWeight:700,marginBottom:12}}>계좌별 안전 설정</div>
