  ...
  return (
    <div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8}}>
          {[
            ...
            <div key={s.l} style={{background:'rgba(0,0,0,.3)',borderRadius:8,padding:'7px 8px',textAlign:'center'}}>
              <div style={{color:s.c,fontSize:11,fontWeight:700,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{s.v}</div>
      ...
      <div style={{display:'flex',gap:5,marginBottom:14,overflowX:'auto'}}>
        {([['dashboard','대시보드'],['settings','설정'],['logic','전략 설명'],['webhook','웹훅'],['pine','📜 Pine Script']] as const).map(([id,l])=>(
                ...
                <div key={sig.id} style={{padding:'11px 14px',borderBottom:i<MOCK_SIGNALS.length-1?`1px solid ${T.border}`:'none'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                    ...
                    <div style={{textAlign:'right',flexShrink:0}}>
                      <div style={{color:T.txt,fontSize:11,fontWeight:700,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{cvt(sig.price,'KRW')}</div>
              ...
              {(Array.isArray(grp.items) ? grp.items : []).map((item,i)=>(
                <div key={item.k} style={{marginBottom:i<grp.items.length-1?12:0}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                    ...
                    <span style={{color:T.acl,fontWeight:800,fontSize:12,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{(settings as any)[item.k]}</span>
                  </div>
        ...
        <div>
          ...
          <Card style={{padding:'14px 16px',marginBottom:12}}>
            ...
            <div style={{marginBottom:8}}>
              ...
              <div style={{display:'flex',gap:8}}>
                <input value={wunderCode} onChange={e=>setWunderCode(e.target.value)} style={{flex:1,background:T.bg,border:`1px solid ${T.border}`,borderRadius:8,padding:'8px 10px',color:T.txt,fontSize:12,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',fontWeight:700,outline:'none'}}/>
              </div>
            </div>
      ...
      {tab==='pine'&&(
        <div>
          ...
          <Card style={{padding:'14px 16px',marginBottom:12}}>
            <div style={{color:T.txt,fontWeight:700,marginBottom:10}}>현재 설정값 → Pine Script 반영</div>
            <div className="mobile-1col" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
              {[
                ...
                <div key={r.l} style={{background:T.alt,borderRadius:7,padding:'7px 9px'}}>
                  ...
                  <div style={{color:T.acl,fontSize:11,fontWeight:700,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',marginTop:1}}>{r.v}</div>
                </div>
              ))}
            </div>
...
const REGIME_INFO: Record<MarketRegime, {label:string;color:string;icon:string;desc:string}> = {
  ...
  range:             {label:'횡보 구간',     color:'#F59E0B',icon:'',desc:'범위 5% 이내, 레인지 전략 적합'},
...
const MOCK_MARKETPLACE: MarketplaceStrat[] = [
  ...
  {id:'ms3',name:'DCA Master 3.0',author:'@dca_master',pnl:15.6,winRate:83,subscribers:987,score:91,type:'dca',color:'#10B981',badge:'안정',verified:true},
