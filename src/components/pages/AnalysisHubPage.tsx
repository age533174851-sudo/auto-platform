          ...
          <Card style={{padding:'14px 16px',border:`1px solid ${T.prp}30`}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:6}}>
              <div style={{background:T.alt,borderRadius:8,padding:'7px',textAlign:'center'}}>
                <div style={{color:T.txt,fontSize:11,fontWeight:700,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>₩1,000,000</div>
...
{tab==='drawing'&&(
      ...
      {tab==='interval'&&(
        <div>
              ...
              <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                {INTERVALS.filter(i=>i.group===grp.id).map(iv=>(
                  <button key={iv.id} onClick={()=>setActiveInterval(iv.id)} style={{background:activeInterval===iv.id?T.acg:T.card,border:`2px solid ${activeInterval===iv.id?T.acl:T.border}`,borderRadius:9,padding:'7px 13px',cursor:'pointer',minWidth:48,textAlign:'center'}}>
                    <span style={{color:activeInterval===iv.id?T.acl:T.txt,fontSize:12,fontWeight:700,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{iv.label}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div style={{display:'flex',gap:6}}>
            <input value={customInterval} onChange={e=>setCustomInterval(e.target.value)} placeholder="직접 입력 (예: 75)" style={{flex:1,background:T.card,border:`1px solid ${T.border}`,borderRadius:9,padding:'9px 12px',color:T.txt,fontSize:12,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',outline:'none'}}/>
            ...
            return (
              <div key={ind.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 0',borderBottom:`1px solid ${T.border}`}}>
                <div style={{display:'flex',gap:8,alignItems:'center'}}>
                  <div style={{width:32,height:32,borderRadius:8,background:active?T.acg:T.alt,border:`1px solid ${active?T.acl:T.border}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,fontWeight:900,color:active?T.acl:T.muted,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{ind.id.slice(0,4)}</div>
                  <div>
          ...
          <div className="mobile-1col" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
            {[{l:'진입가',key:'entry',val:riskEntry,set:setRiskEntry},{l:'손절가',key:'sl',val:riskSL,set:setRiskSL},{l:'목표가',key:'tp',val:riskTP,set:setRiskTP},{l:'투자금',key:'size',val:riskSize,set:setRiskSize}].map(f=>(
              <div key={f.key}>
                ...
                <input type="number" value={f.val} onChange={e=>f.set(+e.target.value)} style={{width:'100%',background:T.bg,border:`1px solid ${T.border}`,borderRadius:8,padding:'8px 10px',color:T.txt,fontSize:11,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',outline:'none'}}/>
              </div>
            ))}
          </div>
          <div style={{marginBottom:12}}>
            ...
            <div className="mobile-1col" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              {[
                ...
                <div key={r.l} style={{background:T.alt,borderRadius:8,padding:'8px 9px'}}>
                  ...
                  <div style={{color:r.c,fontSize:11,fontWeight:700,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{r.v}</div>
                </div>
              ))}
            </div>
          </Card>
        ...
        <BottomSheet title="🌀 피보나치 설정">
          ...
          {fibLevels.map((fib,i)=>(
            <div key={i} style={{display:'flex',gap:8,alignItems:'center',marginBottom:6}}>
              <input type="number" step={0.001} value={fib.level} onChange={e=>setFibLevels(prev=>prev.map((f,j)=>j===i?{...f,level:+e.target.value}:f))} style={{width:70,background:T.bg,border:`1px solid ${T.border}`,borderRadius:7,padding:'5px 8px',color:T.txt,fontSize:11,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',outline:'none'}}/>
      ...
      {showSheet&&sheetContent==='pine'&&(
        <BottomSheet title="🌲 파인 에디터">
          <div style={{background:'#030610',borderRadius:10,padding:'12px',marginBottom:10,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',fontSize:11,color:'#50FA7B',lineHeight:1.7}}>
...
const MTABS=[
  ...
  {id:'posters',label:'강의',icon:''},{id:'safety',label:'안전제어',icon:''},{id:'hub',label:'허브',icon:''},
];
