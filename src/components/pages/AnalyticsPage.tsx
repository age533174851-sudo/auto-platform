...
import type { PlanType, PlanStatus, BadgeType, UserSubscription, InviteCode } from '@/types/domain';
...
function AnalyticsPage({prices,currency}:{prices:Asset[];currency:string}) {
  ...
  const sectorAlloc=[
    ...
    {name:'기타',pct:10,color:'var(--t-sub)'},
  ];
  ...
  return (
    <div>
        ...
        <div>
          ...
          <Card style={{padding:'14px 16px',marginBottom:12,border:`1px solid ${riskScore>70?T.grn:riskScore>40?T.ylw:T.red}30`}}>
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              <div style={{color:riskScore>70?T.grn:riskScore>40?T.ylw:T.red,fontSize:32,fontWeight:900,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{riskScore}</div>
              <div style={{textAlign:'right'}}>
                <Bdg c={T.grn} ch={riskScore>70?'건강':'보통'}/>
                <div style={{color:T.muted,fontSize:10,marginTop:3}}>100점 만점</div>
              </div>
            </div>
            <div style={{height:8,background:'var(--t-border)',borderRadius:4,overflow:'hidden',marginBottom:6}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:8}}>
              {[
                ...
                <div key={m.l} style={{background:T.alt,borderRadius:10,padding:'12px 12px'}}>
                  ...
                  <div style={{color:m.c,fontSize:16,fontWeight:900,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums'}}>{m.v}</div>
      ...
      {tab==='sync'&&(
        <div>
          <Card style={{padding:'14px 16px',marginBottom:12,border:`1px solid ${A(T.grn,'30')}`}}>
            ...
            <button type="button"
              ...
              style={{width:'100%',marginTop:12,padding:'12px',minHeight:44,background:T.acg,color:T.acl,border:`1px solid ${A(T.acl,'40')}`,borderRadius:10,fontWeight:700,fontSize:12,cursor:'pointer'}}>전체 동기화</button>
          </Card>
          <Card style={{padding:'14px 16px',marginBottom:12}}>
            ...
            {['iPhone 15 Pro · 현재 기기','MacBook Pro · 2시간 전'].map((d,i)=>(
              <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 0',borderBottom:i<1?`1px solid ${T.border}`:'none'}}>
                ...
                {i===0?<Bdg c={T.grn} ch="현재" sm/>:<button type="button" onClick={async () => { if ((await confirmDialog(`"${d}" 세션을 해제하시겠습니까?`, { danger: true }))) notifyInfo('설정 → 보안 → 로그인 기록에서 실제 세션 종료 가능'); }} style={{background:A(T.red,'15'),color:T.red,border:'none',borderRadius:6,padding:'5px 10px',minHeight:30,fontSize:10,cursor:'pointer'}}>해제</button>}
              </div>
            ))}
          </Card>
        </div>
            ...
            {[
              ...
              <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:i<4?`1px solid ${T.border}`:'none'}}>
                ...
                <button type="button"
                  ...
                  style={{background:T.acg,color:T.acl,border:`1px solid ${A(T.acl,'40')}`,borderRadius:8,padding:'6px 12px',minHeight:32,fontSize:11,fontWeight:700,cursor:'pointer'}}>↓ 다운로드</button>
              </div>
            ))}
          </Card>
          <Card style={{padding:'14px 16px'}}>
...
// 타입은 @/types/domain 에서 공용 정의 (아래 import 참조)
...
const PLAN_INFO: Record<PlanType,{label:string;color:string;icon:string;price:string;features:string[]}> = {
  ...
  admin:    {label:'관리자',   color:'#10B981',icon:'',price:'내부',        features:['전체 권한','사용자 관리','시스템 접근','코드 생성']},
};
...
const BADGE_INFO: Record<BadgeType,{label:string;color:string;icon:string}> = {
  ...
  admin:     {label:'관리자',  color:'#10B981',icon:''},
