...
function TaxPage({currency}:{currency:string}) {
  ...
  const [tab, setTab]    = useState<'summary'|'history'|'export'|'calculator'>('summary');
  const [loading, setLoading] = useState(false);

  // 참고용 세금 계산기 상태
  const [calcAssetType, setCalcAssetType] = useState<'coin'|'us_stock'|'stock'>('coin');
  const [calcSellPrice, setCalcSellPrice] = useState('');
  const [calcBuyPrice,  setCalcBuyPrice]  = useState('');
  const [calcQty,       setCalcQty]       = useState('');
  const [calcFeeRate,   setCalcFeeRate]   = useState('0.25');

  const taxCalcResult = useMemo(() => {
    const sell = parseFloat(calcSellPrice), buy = parseFloat(calcBuyPrice);
    const qty = parseFloat(calcQty), feeRate = parseFloat(calcFeeRate) / 100;
    if (![sell, buy, qty].every(n => isFinite(n) && n > 0)) return null;

    const totalSell = sell * qty;
    const totalBuy  = buy * qty;
    const totalFee  = Math.round((totalSell + totalBuy) * (isFinite(feeRate) ? feeRate : 0));
    const gain      = Math.round(totalSell - totalBuy - totalFee);
    const gain_pct  = totalBuy > 0 ? (gain / totalBuy) * 100 : 0;

    // 자산 유형별 기본공제 / 세율 (참고용)
    const deduction = calcAssetType === 'us_stock' ? 2_500_000 : calcAssetType === 'coin' ? 2_500_000 : 0;
    const taxRate   = calcAssetType === 'stock' ? 0 : 0.22;
    const taxBase   = Math.max(0, gain - deduction);
    const estTax    = Math.round(taxBase * taxRate);

    return {
      totalSell: Math.round(totalSell), totalBuy: Math.round(totalBuy),
      totalFee, gain, gain_pct, deduction, taxBase, taxRate, estTax,
    };
  }, [calcAssetType, calcSellPrice, calcBuyPrice, calcQty, calcFeeRate]);
