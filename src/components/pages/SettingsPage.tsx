      ...
      {tab==='general'&&(
        <div>
          ...
          <Card style={{padding:16,marginBottom:12}}>
            ...
            <button onClick={()=>{ setNotif(DEFAULT_NOTIF); showToast('알림 설정이 기본값으로 복구되었습니다'); }}
          ...
          <Card style={{padding:16,marginBottom:12}}>
            ...
            {[
              ...
              <div key={a.k} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 0',borderBottom:i<arr.length-1?`1px solid ${T.border}`:'none'}}>
                ...
                <Toggle on={quickActions.includes(a.k)} onChange={v=>{
                  setQuickActions(prev=>{ const next=v?[...prev,a.k]:prev.filter(x=>x!==a.k); try{localStorage.setItem('tg_quick_actions',JSON.stringify(next));}catch{} return next; });
                }}/>
              </div>
            ))}
          ...
          {/* 포트폴리오 리스크 한도 */}
          <Card style={{padding:16,marginBottom:12}}>
            <div style={{color:T.txt,fontWeight:700,marginBottom:4}}>포트폴리오 리스크 한도</div>
            <div style={{color:T.muted,fontSize:10,marginBottom:12}}>전체 기준 손실·노출 제한 (망할 때 덜 망하게)</div>
            {[
              {k:'maxDailyLossPct',l:'하루 최대 손실',unit:'%',min:1,max:50},
              {k:'maxPositions',l:'동시 포지션 수',unit:'개',min:1,max:20},
              {k:'maxExposurePct',l:'총 노출 한도',unit:'%',min:10,max:100},
            ].map((r,i,arr)=>(
              <div key={r.k} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:i<arr.length-1?`1px solid ${T.border}`:'none'}}>
                <span style={{color:T.txt,fontSize:12}}>{r.l}</span>
                <div style={{display:'flex',alignItems:'center',gap:6}}>
                  <input type="text" inputMode="numeric" value={(riskLimits as any)[r.k]}
                    onChange={e=>{ const v=parseInt(e.target.value.replace(/[^0-9]/g,''))||r.min; const next={...riskLimits,[r.k]:Math.max(r.min,Math.min(r.max,v))}; setRiskLimits(next); try{localStorage.setItem('tg_risk_limits',JSON.stringify(next));}catch{} }}
                    style={{width:56,background:T.alt,border:`1px solid ${T.border}`,borderRadius:7,padding:'7px 9px',color:T.txt,fontSize:13,fontWeight:700,textAlign:'right',outline:'none'}}/>
                  <span style={{color:T.muted,fontSize:11,width:16}}>{r.unit}</span>
                </div>
              </div>
            ))}
            <div style={{color:T.muted,fontSize:9,marginTop:8,lineHeight:1.4}}>
              한도 도달 시 신규 진입이 자동 차단됩니다. 하루 손실 한도는 다음 날 리셋됩니다.
            </div>
          </Card>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8}}>
              {[{v:'conservative',l:'보수형',d:'안전 우선',c:T.grn},{v:'balanced',l:'균형형',d:'균형 추구',c:T.ylw},{v:'aggressive',l:'공격형',d:'수익 우선',c:T.red}].map(p=>(
                <button key={p.v}
                  ...
                  <div style={{color:p.c,fontSize:18,marginBottom:4}}>{'보수형'===p.l?'':'균형형'===p.l?'':''}</div>
            ...
            <div key={s.id} style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',padding:'10px 0',borderBottom: i < sessions.length - 1 ? `1px solid ${T.border}` : 'none', gap: 8}}>
              <div style={{flex:1, minWidth:0}}>
                ...
                <div style={{color:T.muted,fontSize:10,marginTop:1}}>
                  {timeLabel}
                  {s.ip_address && <span style={{marginLeft:4,fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums',opacity:0.7}}>· {s.ip_address}</span>}
                </div>
              </div>
              {!isCurrent && (
