  ...
  return (
    <div style={{ ...base, background: `${bg}18`, border: `1px solid ${bg}30` }}>
      ...
      <img
        ...
        loading="lazy"
        decoding="async"
        style={{ width: size * 0.74, height: size * 0.74, objectFit: 'contain', display: loaded ? 'block' : 'none' }}
...
// ── ProviderStatusPanel — fetches real env status from server ──
type ProviderInfo = {
...
function ProviderStatusPanel() {
  ...
  const list = providers ? Object.values(providers) : [
    { name:'Binance', nameKr:'바이낸스', type:'코인', connected:true, note:'Public WebSocket' },
    { name:'ExchangeRate', nameKr:'환율', type:'환율', connected:true, note:'무료 API' },
    { name:'Polygon.io', nameKr:'폴리곤', type:'미국주식', connected:false, note:'POLYGON_API_KEY 필요' },
    { name:'Finnhub', nameKr:'핀허브', type:'미국주식', connected:false, note:'FINNHUB_API_KEY 필요' },
    { name:'KIS Open API', nameKr:'한투', type:'한국주식', connected:false, note:'KIS_APP_KEY 필요' },
    { name:'Commodity API', nameKr:'원자재', type:'원자재', connected:false, note:'COMMODITY_API_KEY 필요' },
  ];

  return (
    <div style={{ padding: '14px 16px' }}>
      <div style={{ color: T.muted, fontSize: 11, fontWeight: 700, marginBottom: 10 }}>🌐 데이터 제공사 연결 현황</div>
      <div className="mobile-1col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        {list.map((p, i) => (
          <div key={i} style={{
            background: T.alt, borderRadius: 8, padding: '8px 10px',
            borderLeft: `3px solid ${p.connected ? T.grn : T.border}`,
          }}>
            <div style={{ color: T.txt, fontSize: 11, fontWeight: 600 }}>{p.nameKr || p.name}</div>
            <div style={{ color: T.muted, fontSize: 9 }}>{p.type}</div>
            <div style={{ color: p.connected ? T.grn : T.ylw, fontSize: 9, marginTop: 2, fontWeight: 700 }}>
              {p.connected ? '✅ 연결됨' : '🔑 API키 필요'}
            </div>
          </div>
        ))}
      </div>
      <div style={{ color: T.muted, fontSize: 10, marginTop: 10, lineHeight: 1.5 }}>
        💡 Vercel 환경변수에 API 키를 추가하면 실시간 데이터가 활성화됩니다.
      </div>
    </div>
  );
}
...
export function RiskDashboard({ positions, prices }: { positions: any[]; prices: Asset[] }) {
  ...
  const stressedPnl = positions.reduce((s, p) => {
    ...
    const priceDrop = stressPcts[stressMode] / 100;
    const pnl = a ? p.invested * priceDrop * (p.type === 'short' ? -1 : 1) : 0;
    return s + pnl;
  }, 0);
    ...
    <div style={{ background: T.alt, borderRadius: 10, padding: '10px 12px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <span style={{ color: T.red, fontWeight: 800, fontSize: 13 }}>
          {stressedPnl.toLocaleString(undefined, { maximumFractionDigits: 0 })}
        </span>
      </div>
      {totalInvested > 0 && (
        <div style={{ color: T.muted, fontSize: 10, marginTop: 4, textAlign: 'right' }}>
          ({((stressedPnl / totalInvested) * 100).toFixed(1)}% of portfolio)
        </div>
      )}
    </div>
  );
...
export function InlineTVChart({ symbol, chartType = '1', interval = '60' }: {
  symbol: string; chartType?: string; interval?: string;
}) {
  // Convert symbol to TradingView format
  const tvSymbol = (() => {
    if (!symbol) return 'BINANCE:BTCUSDT';
    const s = symbol.toUpperCase().replace(/[-/]/g, '').replace('USDT', '');
    // Crypto coins
    const cryptos: Record<string,string> = {
      BTC:'BINANCE:BTCUSDT', ETH:'BINANCE:ETHUSDT', SOL:'BINANCE:SOLUSDT',
      XRP:'BINANCE:XRPUSDT', BNB:'BINANCE:BNBUSDT', DOGE:'BINANCE:DOGEUSDT',
      ADA:'BINANCE:ADAUSDT', AVAX:'BINANCE:AVAXUSDT', TON:'BINANCE:TONUSDT',
      ...
      ARB:'BINANCE:ARBUSDT', OP:'BINANCE:OPUSDT', LTC:'BINANCE:LTCUSDT',
    };
    if (cryptos[s]) return cryptos[s];
    // US stocks
    const nasdaqStocks = ['AAPL','MSFT','NVDA','TSLA','GOOGL','GOOG','AMZN','META','AMD','INTC',
      'AVGO','QCOM','PLTR','COIN','MSTR','RIVN','SNOW','CRWD','SHOP','ORCL','ADBE','NFLX'];
    if (nasdaqStocks.includes(s)) return `NASDAQ:${s}`;
    // ETFs
    const amexEtfs = ['SPY','QQQ','IWM','DIA','TQQQ','SQQQ','SOXL','SOXS','ARKK','GLD','TLT','USO'];
    if (amexEtfs.includes(s)) return `AMEX:${s}`;
    // Korean stocks (6-digit)
    if (/^\d{6}$/.test(s)) return `KRX:${s}`;
    // Default: try as crypto
    return `BINANCE:${s}USDT`;
  })();

  const src = `https://s.tradingview.com/widgetembed/?frameElementId=tv_embed&symbol=${encodeURIComponent(tvSymbol)}&interval=${interval}&theme=dark&style=${chartType}&locale=ko&hide_top_toolbar=0&hide_legend=0&save_image=0&toolbarbg=060B14&studies=[]&show_popup_button=0`;
