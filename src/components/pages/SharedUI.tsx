...
export function WorldClock() {
  ...
  const isOpen=(mk:typeof WORLD_MARKETS[0])=>{
    ...
    try{
      ...
      const mins=h*60+mi;
      const toMins=(v:number|string|undefined)=>{
        if(typeof v==='number'){return Math.floor(v/100)*60+(v%100);}
        if(typeof v==='string'){const [hh,mm]=v.split(':').map(Number);return hh*60+(mm||0);}
        return 0;
      };
      const openMins=toMins(mk.open);const closeMins=toMins(mk.close);
    ...
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
        {WORLD_MARKETS.map(mk=>{const open=isOpen(mk);return(
          <div key={mk.name} style={{background:open?(mk as any).color ? (mk as any).color+'15':T.alt,border:`1px solid ${open?(mk as any).color??T.acl:T.border}`,borderRadius:12,padding:'10px 12px'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
              <span style={{color:T.txt,fontWeight:700,fontSize:11}}>{mk.flag} {mk.name}</span>
              <span style={{background:open?(mk as any).color ? (mk as any).color+'25':'rgba(71,85,105,.3)',color:open?(mk as any).color??T.acl:T.muted,fontSize:9,fontWeight:700,borderRadius:99,padding:'1px 6px'}}>{open?'개장':'폐장'}</span>
            </div>
            <div style={{color:open?(mk as any).color??T.acl:T.muted,fontFamily:'monospace',fontSize:13,fontWeight:800}}>{mk.name==='코인'?'24/7':getTime(mk.tz)}</div>
            <div style={{color:T.muted,fontSize:9,marginTop:2}}>{mk.open!=null?`${Math.floor((mk.open as number)/100)}:${String((mk.open as number)%100).padStart(2,"0")}`:""}–{mk.close!=null?`${Math.floor((mk.close as number)/100)}:${String((mk.close as number)%100).padStart(2,"0")}`:""}</div>
          </div>
        );})}
      </div>
    </div>
...
export function InlineTVChart({ symbol, chartType='1', interval='60' }: {
  ...
  const wid = React.useRef<any>(null);
  React.useEffect(() => {
    if (!ref.current || !symbol) return;
    const w: any = window;
    if (!w.TradingView) return;
    wid.current = new w.TradingView.widget({
      container_id: ref.current.id || 'tv_widget',
      symbol, interval,
      theme: 'dark', style: chartType,
      locale: 'ko', width: '100%', height: '100%',
      hide_top_toolbar: false, hide_legend: false, save_image: false,
    });
    return () => { try { wid.current?.remove?.(); } catch {} };
  }, [symbol, interval, chartType]);
  if (!symbol) return (
    <div style={{height:'100%',display:'flex',alignItems:'center',justifyContent:'center',
                 flexDirection:'column',gap:8,color:'#64748B'}}>
      <span style={{fontSize:24}}>📊</span>
      <span style={{fontSize:12}}>심볼을 선택하세요</span>
    </div>
  );
