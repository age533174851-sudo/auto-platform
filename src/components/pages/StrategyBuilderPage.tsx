...
function StrategyList({
  ...
  // ── 이 전략들이 어디 사는가 ──
  //
  // AutoTradeEngine이 이 전략들을 주기적으로 평가한다(폴링 60초, 앞 tick이
  // 안 끝나면 건너뜀, 신호 후 5분 쿨다운). 그런데 전략은
...
function ManualBuilder({
  ...
  // 현재 시장상태(레짐) 로드 — 자동 전환 추천
  useEffect(() => {
    let cancelled = false;
    const sym = (s.asset || 'BTC').toUpperCase().replace(/USDT$/, '') + 'USDT';
    fetch(`/api/regime?symbol=${sym}`)
      .then(r => r.json())
      .then(d => { if (!cancelled && d.regime) setRegime(d); })
      .catch(() => {});
    return () => { cancelled = true; };
  }, [s.asset]);
    ...
    <div>
      ...
      {step === 5 && (
        <div style={cardStyle()}>
          ...
          {/* ── 고급 손실 방어 ── */}
          <div style={{ marginTop: SP.md, paddingTop: SP.md, borderTop: `1px solid ${T.border}` }}>
            <div style={{ ...F.section, marginBottom: SP.sm, color: T.acl }}>고급 손실 방어</div>

            {/* 1. 고정 리스크 모델 */}
            <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: R.md, padding: SP.sm, marginBottom: SP.sm }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ color: T.txt, fontWeight: 700, fontSize: 12 }}>고정 리스크 모델</div>
              ...
              {s.risk.useFixedRisk && (
                <div style={{ marginTop: SP.sm, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {[
                    { k: 'riskPerTradePct', l: '1회 리스크 (%)', def: 2, min: 0.5, max: 10, step: 0.5, c: T.red },
                    { k: 'atrMultiplier',   l: 'ATR 손절 배수',  def: 1.5, min: 0.5, max: 5, step: 0.1, c: T.ylw },
                    { k: 'maxLeverage',     l: '최대 레버리지',  def: 10, min: 1, max: 50, step: 1, c: T.prp },
                  ].map(({ k, l, def, min, max, step, c }) => (
                    <div key={k}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                        <span style={{ color: T.muted, fontSize: 10 }}>{l}</span>
                        <span style={{ color: c, fontWeight: 700, fontSize: 11 }}>{(s.risk as any)[k] ?? def}{k === 'maxLeverage' ? 'x' : k === 'atrMultiplier' ? '' : '%'}</span>
                      </div>
                      <input type="range" min={min} max={max} step={step} value={(s.risk as any)[k] ?? def}
                        onChange={e => update({ risk: { ...s.risk, [k]: parseFloat(e.target.value) } })}
                        style={{ width: '100%', accentColor: c }}/>
                    </div>
                  ))}
                  <div style={{ color: T.muted, fontSize: 9, lineHeight: 1.4 }}>
                    변동성(ATR)에 맞춰 수량 자동 조절. 레버리지는 직접 지정이 아닌 역산값. 켜면 익절/손절%도 ATR 기반 자동 적용.
                  </div>
                </div>
              )}
            </div>
            ...
            {/* 2. 시장상태 필터 */}
            <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: R.md, padding: SP.sm }}>
              <div style={{ color: T.txt, fontWeight: 700, fontSize: 12, marginBottom: 2 }}>시장상태 필터</div>
              <div style={{ color: T.muted, fontSize: 9, marginBottom: 8 }}>나쁜 장에서 자동 진입 차단 (급락장·고변동장)</div>

              {/* 현재 시장상태 실시간 표시 */}
              {regime?.regime && (() => {
                const rg = regime.regime;
                const tc = rg.trend === 'uptrend' ? T.grn : rg.trend === 'downtrend' ? T.red : T.ylw;
                return (
                  <div style={{ background: tc + '12', border: `1px solid ${tc}30`, borderRadius: R.sm, padding: '8px 10px', marginBottom: 8 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                      <span style={{ color: T.muted, fontSize: 9 }}>현재 {(s.asset || 'BTC')} 시장</span>
                      <span style={{ color: tc, fontWeight: 800, fontSize: 11 }}>{rg.label}</span>
                    </div>
                    <div style={{ color: T.sub, fontSize: 9, lineHeight: 1.4 }}>
                      추천: {regime.recommendedStrategy}
                    </div>
                  </div>
                );
              })()}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                {([
                  { v: 'any',          l: '제한 없음',   d: '항상 진입' },
                  { v: 'avoid_highvol',l: '고변동 회피', d: '급변동 차단' },
                  { v: 'trend_only',   l: '추세장 전용', d: '횡보 차단' },
                  { v: 'range_only',   l: '횡보장 전용', d: '추세 차단' },
                ] as const).map(opt => {
                  const active = (s.marketFilter || 'any') === opt.v;
                  return (
                    <button key={opt.v} onClick={() => update({ marketFilter: opt.v })}
                      ...
                      <span style={{ color: T.muted, fontSize: 8 }}>{opt.d}</span>
                    </button>
                  );
                })}
              </div>
              <div style={{ color: T.muted, fontSize: 9, marginTop: 6, lineHeight: 1.4 }}>
                고변동장(ATR≥4%)·하락장 롱·상승장 숏은 항상 차단됩니다.
              </div>
            </div>

            {/* 3. 경제지표 회피 */}
            <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: R.md, padding: SP.sm, marginTop: SP.sm }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ color: T.txt, fontWeight: 700, fontSize: 12 }}>경제지표 회피</div>
