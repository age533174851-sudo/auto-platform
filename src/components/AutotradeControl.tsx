...
// ── 3층으로 나눈 조각들 ──
//
// 홈(돈·전략) / 전략 설정(매매 파라미터) / 시스템 진단(Worker·DB·대조).
// 셋이 한 파일에 섞여 있던 것이 이 화면의 가장 큰 빚이었다.
import AutoSheet, { AutoSheetRow } from './auto/AutoSheet';
import AutoPortfolioHero from './auto/AutoPortfolioHero';
import ActiveStrategyCard from './auto/ActiveStrategyCard';
import EmptyStrategyState from './auto/EmptyStrategyState';
import KillSwitchSheet from './auto/KillSwitchSheet';
import { autoHomeView } from '@/lib/ui/autoHome';
...
export default function AutotradeControl() {
  ...
  /** 지금 열린 판. **한 번에 하나만 연다** */
  const [sheet, setSheet] = useState<'SETUP' | 'SCHEDULES' | 'HEALTH' | 'KILL' | null>(null);
  /** 지갑 개요. 홈의 자산·오늘 손익이 여기서 온다. **없으면 null이다** */
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // ── 자산·오늘 손익 ──
      //
      // 예약 API는 예약만 안다. 홈의 큰 숫자는 지갑 개요에서 온다.
      // **실패해도 화면을 죽이지 않는다** — 그 칸이 "아직 못 읽었습니다"가
      // 될 뿐이고, 0으로 그리지 않는다.
      try {
        const w = await fetch('/api/wallets/overview', { headers: { Authorization: auth } });
        setWallet(await w.json().catch(() => null));
  ...
  // ── 홈이 쓸 값 ──
  //
  // **여기서 숫자를 만들지 않는다.** 아는 것과 모르는 것을 가르는 일은
  // `autoHomeView`가 하고, 모르는 칸은 숫자 대신 이유가 온다.
  // 큰 글씨로 적힌 0은 작은 글씨로 적힌 0보다 나쁘다.
  const home = autoHomeView({ env, schedule: data, wallet });

  // 접힌 줄에 적을 한 줄. **정상일 때만 '정상'이라고 적는다** —
  // 확인 못 한 것은 통과가 아니다.
  const healthLine = checks.blockingCount > 0
    ? `확인 필요 ${checks.blockingCount}건`
    : checks.unknown > 0
      ? `확인 못 함 ${checks.unknown}건`
      : `정상 ${checks.ok}/${checks.total}`;
  ...
  return (
    <div style={box}>
      {/* ── ① 사용자용 자동매매 홈 ── 돈부터 보여준다 */}
      <AutoPortfolioHero view={home} toneColor={toneColor} />

      {/* 돈에 영향을 주는 이상은 접지 않는다 — 홈 맨 위에 남는다 */}
      {/* ── 문제가 있을 때만 뜨는 경고 ── */}
      {alerts.map(a => (
        <div key={a.id} style={{
      ...
      {/* ── 내 자동매매 ──
          예약 한 줄을 서버 내부 상태 나열이 아니라 카드로 그린다.
          자세한 것은 눌렀을 때(예약 상세 판) 나온다. */}
      {home.cards.length > 0 ? (
        <div style={{ marginBottom: 10 }}>
          <div style={{ color: T.muted, fontSize: 10.5, fontWeight: 800, marginBottom: 6 }}>내 자동매매</div>
          {home.cards.map(c => (
            <ActiveStrategyCard key={c.scheduleId} card={c} nowMs={Date.now()} onOpen={() => setSheet('SCHEDULES')} />
          ))}
        </div>
      ) : home.schedulesReadOk ? (
        <EmptyStrategyState onStart={() => { setLive(false); setSheet('SETUP'); }} />
      ) : null}

      {/* ── ② 전략 설정 · ③ 시스템 진단 ──
          지우는 것이 아니라 옮긴다. 진단 정보는 돈에 문제가 생겼을 때
          가장 먼저 필요하고, 없애면 그때 사람이 Fly 로그를 연다. */}
      <AutoSheetRow title="전략 설정" summary="연결 · 전략 · 위험 · 배율 · 주기"
        onOpen={() => setSheet('SETUP')} />
      <AutoSheetRow title="시스템 상태" summary={healthLine}
        tone={checks.blockingCount > 0 ? T.red : null} onOpen={() => setSheet('HEALTH')} />

      {/* ── 전체 정지 ──
          **일반 탭 옆에 큰 빨간 버튼을 늘 두지 않는다.** 매번 눈에
          익으면 위험성이 떨어지고, 그때 잘못 눌린다. 누르면 바로
          실행되지도 않는다 — 판이 올라오고 밀어야 나간다. */}
      <button onClick={() => setSheet('KILL')} style={{
        width: '100%', minHeight: 44, marginTop: 4, cursor: 'pointer',
        background: 'transparent', color: T.red,
        border: `1px solid ${A(T.red, '35')}`, borderRadius: 12,
      ...
      {/* ── 판 ── 기존 내용을 그대로 옮겼다. 지운 것은 없다 */}
      <AutoSheet open={sheet === 'SETUP'} title="전략 설정" onClose={() => setSheet(null)}>
          {/* 새로 켜기 */}
      ...
      <AutoSheet open={sheet === 'SCHEDULES'} title="예약 상세"
        summary={`${home.cards.length}건`} onClose={() => setSheet(null)}>
          {/* ── 예약 ──
      ...
      <AutoSheet open={sheet === 'HEALTH'} title="시스템 상태"
        summary={healthLine} tone={checks.blockingCount > 0 ? T.red : null}
      ...
      <KillSwitchSheet open={sheet === 'KILL'} auth={auth} connectionId={connId}
        onClose={() => setSheet(null)} onDone={() => load()} />
