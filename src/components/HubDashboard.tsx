...
// ─── Market Cap Widget ───────────────────────────────────────────
function MarketCapWidget({ currency }: { currency: string }) {
  ...
  return (
    <div>
      <SectionTitle emoji="🏆" title="시가총액 TOP 10" sub="글로벌 메가캡 기업"/>
      ...
      {loading ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          {data.map((item, i) => (
            <Card key={item.ticker} style={{ padding:'12px 14px' }}>
...
function CopyTradingWidget() {
  ...
  return (
    <div>
      <SectionTitle emoji="👑" title="수익률 리더보드" sub="고수 전략 따라하기 · 복사 거래"/>
...
function AIStrategyBuilder() {
  ...
  return (
    <div>
      <SectionTitle emoji="🤖" title="AI 전략 빌더" sub="자연어로 전략 설명 → 자동 생성"/>
...
function MarketBriefing() {
  ...
  return (
    <div>
      <SectionTitle emoji="📡" title="AI 시장 브리핑" sub="일일 시장 요약 · 이벤트 영향 분석"/>
      {loading ? (
        ...
        <Card style={{ marginBottom:10, background:'linear-gradient(135deg,#0D1A35,#091228)', border:`1px solid ${T.acl}30` }}>
          ...
          <div className="mobile-1col" style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6 }}>
            {[
              { label:'BTC', value:`₩${Math.round((data.btcPrice||0)/10000)}만`, color:(data.btcChg||0)>=0?T.grn:T.red },
              { label:'펀딩 편향', value:data.fundBias||'중립', color:T.ylw },
              { label:'시장 감성', value:data.sentiment||'NEUTRAL', color:T.acl },
            ].map(m => (
...
function LiquidationMap() {
  ...
  if (loading) return (
    <div>
      <SectionTitle emoji="🌡️" title="청산 히트맵" sub="롱/숏 청산 구간 분석"/>
...
function PortfolioRebalance({ currency }: { currency: string }) {
  ...
  return (
    <div>
      <SectionTitle emoji="⚖️" title="AI 포트폴리오 리밸런싱" sub="동적 자산 배분 최적화"/>
...
export default function HubDashboard({ currency = 'KRW' }: { currency?: string }) {
  ...
  return (
    <div style={{ color: T.txt }}>
      ...
      {section === 'mcap'     && <MarketCapWidget currency={currency}/>}
