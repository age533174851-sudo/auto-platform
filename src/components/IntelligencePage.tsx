...
const TABS: { id:TabId; label:string; icon: LucideIcon }[] = [
  { id:'onchain',   label:'온체인',     icon: Link2 },
  { id:'calendar',  label:'경제캘린더', icon: CalendarDays },
  { id:'voice',     label:'AI 브리핑',  icon: Mic },
  { id:'accounts',  label:'계좌관리',   icon: Wallet },
  { id:'tax',       label:'손익리포트', icon: Receipt },
  { id:'simulator', label:'시뮬레이터', icon: Dice5 },
  { id:'community', label:'커뮤니티',   icon: Users },
  { id:'assistant', label:'AI 비서',    icon: Bot },
];
...
function OnChainTab() {
  ...
  return (
    <div>
      <SHead title="온체인 데이터" sub={`${data.source === 'mock' ? '모의 데이터 · ' : ''}${new Date(data.timestamp).toLocaleTimeString('ko-KR')}`}/>
      ...
      <Card c={<>
        ...
        <div className="mobile-1col" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, marginBottom:6 }}>
      ...
      <Card c={<>
        ...
        {(Array.isArray(data.whales) ? data.whales : []).map((w: any, i: number) => (
          <div key={i} style={{ display:'flex', gap:8, padding:'7px 0', borderBottom: i < data.whales.length-1 ? `1px solid ${T.border}` : 'none', alignItems:'center' }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ color:T.txt, fontSize:10 }}><span style={{ color:T.acl, fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums' }}>{w.address}</span></div>
...
function CalendarTab() {
  ...
  return (
    <div>
      <SHead title="경제 캘린더" sub="CPI · FOMC · NFP · 옵션 만기"/>
      ...
      {loading ? <Skeleton h={200}/> : filtered.map(ev => (
        <Card key={ev.id} c={<>
          ...
          <div className="mobile-1col" style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:4, marginBottom:6 }}>
          ...
          <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
            {(Array.isArray(ev.assets) ? ev.assets : []).map((a: string) => <Badge key={a} label={a} color={T.acl}/>)}
          </div>
...
function VoiceTab() {
  ...
  return (
    <div>
      <SHead title="AI 음성 브리핑" sub="오늘 시장 요약 · TTS 자동 읽기"/>
      <Card c={<>
...
function AccountsTab({ prices }: { prices?: any[] }) {
  ...
  return (
    <div>
      <SHead title="멀티 계좌 관리" sub="여러 거래소 통합 관리"/>
      <Card c={<>
...
function TaxTab({ currency }: { currency?: string }) {
  ...
  return (
    <div>
      <SHead title="손익 · 세금 리포트" sub="월별 P&L · 수수료 · 예상 세금"/>
      {/* Year picker */}
      ...
      {data && <>
        ...
        <Card c={<>
          ...
          <div style={{ overflowX:'auto' }}>
            {(Array.isArray(data.monthlyData) ? data.monthlyData : []).map((m: any) => (
              <div key={m.month} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:`1px solid ${T.border}` }}>
                ...
                <span style={{ color:m.pnl>=0?T.grn:T.red, fontSize:10, fontFamily:'Inter,monospace',fontVariantNumeric:'tabular-nums', width:70, textAlign:'right', flexShrink:0 }}>
...
function SimulatorTab() {
  ...
  return (
    <div>
      <SHead title="전략 시뮬레이터" sub="몬테카를로 · 1년 전에 이 전략 사용했다면?"/>
      <Card c={<>
...
function AssistantTab() {
  ...
  return (
    <div>
      <SHead title="AI 트레이딩 비서" sub="손실 분석 · 전략 추천 · 리스크 진단"/>
      <Card c={<>
          ...
          return (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              style={{ flexShrink:0, display:'flex', alignItems:'center', gap:5, padding:'7px 12px',
                ...
                borderRadius:10, fontSize:10, fontWeight:700, cursor:'pointer', whiteSpace:'nowrap', minHeight:34 }}>
