...
async function generateBriefing() {
  ...
  try {
    ...
    if (t.status === 'fulfilled') { btcPrice = parseFloat(t.value.lastPrice) * KRW; btcChg = parseFloat(t.value.priceChangePercent); }
    if (f.status === 'fulfilled') { fundingRate = parseFloat(f.value.lastFundingRate || '0.0001'); }
  } catch {}
