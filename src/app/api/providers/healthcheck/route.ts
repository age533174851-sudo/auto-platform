...
function diagnose(code: number): { status: Status; detail: string } {
  ...
  if (code === 401) return { status: 'error', detail: 'API 키 무효 (Invalid Key)' };
  if (code === 403) return { status: 'limited', detail: 'API 키 만료·권한 부족 — 백업 소스로 자동 전환됨' };
  ...
  try {
    ...
    if (!r.ok) {
      ...
      if (r.status === 451 || r.status === 403) {
        ...
        return { id:'binance', name:'Binance', nameKr:'바이낸스', type:'코인', status:'limited', latencyMs:lat, detail:'서버 지역 차단 (HTTP 403/451) — 사용자 브라우저에선 정상일 수 있음. CoinGecko로 자동 전환됨' };
      }
