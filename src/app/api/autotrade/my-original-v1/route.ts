...
//
// 이 전략은 실제로 주문을 낸다 (TESTNET)
// ────────────────────────────────────
// **예전 주석은 "평가만 하고 진입하지 않는다"였다. 더 이상 사실이 아니다.**
//
// 그때 비어 있던 둘이 지금은 들어와 있다:
//
//   1. 방향 — `originalV1Signal`이 09:10~09:30 구간의 시가·종가·고저로
//      롱/숏을 정한다. 구간을 덮는 봉이 없으면 방향을 만들지 않는다
//   2. 손절·익절 — `exitPolicy`가 준다. `protectionPolicy: 'REQUIRED'`라
//      **손절을 못 걸면 방금 연 포지션을 즉시 되돌린다**
//
// 낡은 주석을 그대로 두는 것이 왜 위험한가: 이 파일을 여는 사람은
// "이건 아직 안 도는 전략"이라고 읽고, 실제로는 100배 주문이 나가는
// 코드를 안심하고 고친다. **코드보다 주석이 먼저 거짓말한다.**
//
// 실전(LIVE)은 아직 아니다
// ────────────────────────
// 아래 `executeOrder` 호출은 `mode: 'TESTNET'`으로 고정돼 있다.
// 저장소 규칙 그대로다 — **LIVE는 모든 TESTNET 검증이 끝나기 전에
// 활성화하지 않는다.** 바꾸려면 이 한 줄이 아니라 승격 절차를 거쳐야 한다.
