// /api/binance/futures/tpsl — jobs 큐에 SET_TPSL 적재 (Worker 실행)
...
export async function POST(req: NextRequest) {
  ...
  const { connectionId, symbol, positionSide, tpPrice, slPrice } = body;
  if (!connectionId || !symbol || !positionSide) return NextResponse.json({ error: 'missing_params' }, { status: 400 });
  ...
  const r = await enqueueJob(sb, {
    userId: uid, connectionId, action: 'SET_TPSL',
    mode: conn.is_testnet ? 'TESTNET' : 'LIVE', symbol, side: positionSide,
    payload: { tpPrice: tpPrice != null ? Number(tpPrice) : null, slPrice: slPrice != null ? Number(slPrice) : null },
    priority: 3,
  });
