...
export async function GET(req: NextRequest) {
  ...
  if (data.length === 0 && eodKey) {
    try {
      source = 'eodhd';
      const results = await Promise.allSettled(
        stocks.map(stock =>
          fetch(`https://eodhd.com/api/real-time/${stock.id}.US?api_token=${eodKey}&fmt=json`, { signal: AbortSignal.timeout(4000) })
            .then(r => r.ok ? r.json() : null)
            .then(d => ({ stock, d }))
        )
      );
      for (const res of results) {
        if (res.status !== 'fulfilled' || !res.value.d) continue;
        const { stock, d } = res.value;
        const usd = parseFloat(d.close) || 0;
        const ch  = parseFloat(d.change_p) || 0;
        const m   = MOCK_USD[stock.id] ?? { p: 0, c: 0 };
        data.push({ ...stock, price: Math.round((usd || m.p) * krw), priceUsd: +(usd || m.p).toFixed(2),
          change24h: +(ch || m.c).toFixed(2), category:'stock', status: usd > 0 ? 'live' : 'mock' });
      }
    } catch { /* fall to mock */ }
  }
  ...
  return NextResponse.json({ ok:true, source, status: source==='mock'?'mock':'live',
    failoverChain: ['polygon','finnhub','eodhd','mock'],
    activeSource: source,
