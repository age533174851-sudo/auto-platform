...
async function getSessionAndRole(): Promise<{userId:string|null;role:string|null;token:string|null}> {
  try {
    ...
    return {userId:session.user.id, role:profile?.role??'user', token:session.access_token};
...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<'users'|'exchange'|'health'|'strategies'|'audit'|'system'>('health');
