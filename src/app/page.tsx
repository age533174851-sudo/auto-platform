...
const TV_FEATURED = [
  {sym:'BTC', label:'비트코인', tv:'BINANCE:BTCUSDT', cat:'crypto',  clr:'#F7931A'},
  {sym:'ETH', label:'이더리움', tv:'BINANCE:ETHUSDT', cat:'crypto',  clr:'#627EEA'},
  {sym:'SOL', label:'솔라나',   tv:'BINANCE:SOLUSDT', cat:'crypto',  clr:'#9945FF'},
  {sym:'NVDA',label:'엔비디아', tv:'NASDAQ:NVDA',      cat:'stock',   clr:'#76B900'},
  {sym:'AAPL',label:'애플',     tv:'NASDAQ:AAPL',      cat:'stock',   clr:'#555555'},
  {sym:'TSLA',label:'테슬라',   tv:'NASDAQ:TSLA',      cat:'stock',   clr:'#CC0000'},
  {sym:'MSFT',label:'마이크로소프트',tv:'NASDAQ:MSFT', cat:'stock',   clr:'#00A4EF'},
  {sym:'SEC', label:'삼성전자', tv:'KRX:005930',        cat:'krstock', clr:'#1428A0'},
  {sym:'QQQ', label:'나스닥 ETF',tv:'NASDAQ:QQQ',      cat:'etf',     clr:'#3B82F6'},
  {sym:'SPX', label:'S&P500',   tv:'SP:SPX',            cat:'index',   clr:'#6366F1'},
  {sym:'GOLD',label:'금',       tv:'OANDA:XAUUSD',      cat:'commodity',clr:'#D97706'},
  {sym:'DXY', label:'달러',     tv:'TVC:DXY',           cat:'forex',   clr:'#10B981'},
];
...
function InlineTVChart({ symbol }: { symbol:string }) {
  const ref = useRef<HTMLDivElement>(null);
  const wid = useRef<any>(null);

  useEffect(() => {
    if (!ref.current || typeof window === 'undefined') return;
    const uid = 'tv_inline_' + Date.now().toString(36);
    ref.current.id = uid;
    if (wid.current) { try { wid.current.remove?.(); } catch {} wid.current = null; }
    ref.current.innerHTML = '';

    const init = () => {
      if (!ref.current) return;
      try {
        wid.current = new window.TradingView.widget({
          container_id: uid, autosize: true,
          symbol, interval:'60', timezone:'Asia/Seoul',
          theme:'dark', style:'1', locale:'kr',
          toolbar_bg:'#0F1924', enable_publishing:false,
          allow_symbol_change:true, save_image:false,
          hide_side_toolbar:false, withdateranges:true, details:false,
          studies:['RSI@tv-basicstudies','MASimple@tv-basicstudies'],
          backgroundColor:'#060B14', gridColor:'rgba(26,45,74,0.5)',
    ...
    if (window.TradingView) { init(); }
      ...
      const iv = setInterval(()=>{ if(window.TradingView){ clearInterval(iv); init(); }},300);
    } else {
      const s = document.createElement('script');
      s.id = scriptId; s.src='https://s3.tradingview.com/tv.js'; s.async=true;
      s.onload = init; document.head.appendChild(s);
    }
    return () => { if(wid.current){ try{wid.current.remove?.();}catch{} wid.current=null; } };
  }, [symbol]);
...
function ChartTab() {
  const [sel,setSel]=useState({sym:'BTC',label:'비트코인',tv:'BINANCE:BTCUSDT',cat:'crypto',clr:'#F7931A'});
  const [sel2,setSel2]=useState({sym:'NVDA',label:'엔비디아',tv:'NASDAQ:NVDA',cat:'stock',clr:'#76B900'});
  const [query,setQuery]=useState('');
    ...
    async function fetchData(){
      try{
        ...
        for(const item of (json.data||[])){
          map[item.symbol]={price:item.price,change:item.change24h};
        }
        setLiveData(map);
      }catch{}
    }
  ...
  const featured=catFilt==='all'?TV_FEATURED:TV_FEATURED.filter(f=>f.cat===catFilt);
  const filtered=query?TV_FEATURED.filter(f=>f.sym.toLowerCase().includes(query.toLowerCase())||f.label.includes(query)||f.tv.toLowerCase().includes(query.toLowerCase())):featured;

  const applyManual=()=>{
    if(!manualSym.trim()) return;
    const tv=tvSymbol(manualSym);
    setSel({sym:manualSym.toUpperCase(),label:manualSym.toUpperCase(),tv,cat:'stock',clr:T.acl});
    setManualMode(false);setManualSym('');setQuery('');
  };

  const selBase=sel.sym.replace('USDT','');
    ...
    <div>
      ...
      <div style={{background:'linear-gradient(135deg,#060B14,#0A0F1E)',border:`1px solid ${sel.clr}40`,borderRadius:18,padding:'14px 16px',marginBottom:12}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
          <div>
            <div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap',marginBottom:3}}>
              <div style={{width:28,height:28,borderRadius:8,background:`${sel.clr}20`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:900,color:sel.clr}}>{sel.sym.slice(0,3)}</div>
              <span style={{color:T.txt,fontWeight:800,fontSize:15}}>{sel.label}</span>
              <span style={{background:`${TV_CAT_COLOR[sel.cat]||T.acl}20`,color:TV_CAT_COLOR[sel.cat]||T.acl,fontSize:9,fontWeight:700,padding:'2px 7px',borderRadius:99}}>{sel.cat}</span>
              {provStatus&&(
                <span style={{background:provStatus.status==='live'?T.grn+'20':T.ylw+'20',color:provStatus.status==='live'?T.grn:T.ylw,fontSize:8,fontWeight:700,padding:'2px 6px',borderRadius:99}}>
                  {provStatus.status==='live'?'● LIVE':'● MOCK'} {provStatus.source} {provStatus.latency}ms
                </span>
              )}
            </div>
            <div style={{color:T.muted,fontSize:10,fontFamily:'monospace'}}>{sel.tv}</div>
          </div>
          <div style={{textAlign:'right',flexShrink:0}}>
            {selLive&&(
              <div>
                <div style={{color:T.txt,fontWeight:700,fontSize:14,fontFamily:'monospace'}}>
                  {selLive.price>=1000?selLive.price.toLocaleString('ko-KR',{maximumFractionDigits:0})+'원':selLive.price.toFixed(4)}
                </div>
                <div style={{color:selLive.change>=0?T.grn:T.red,fontSize:11,fontWeight:700}}>
        ...
        <div style={{display:'flex',gap:5,alignItems:'center'}}>
          ...
          <a href="/chart" target="_blank" style={{marginLeft:'auto',background:T.acg,color:T.acl,border:`1px solid ${T.acl}40`,borderRadius:8,padding:'4px 10px',fontSize:10,fontWeight:700,textDecoration:'none'}}>⛶ 전체화면 &amp; 4분할 ↗</a>
        </div>
      </div>
      ...
      {layout==='2h'&&(
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginBottom:12}}>
          <div style={{height:360,borderRadius:10,overflow:'hidden',border:`1px solid ${T.border}`,background:T.card}}><InlineTVChart symbol={sel.tv}/></div>
          <div style={{height:360,borderRadius:10,overflow:'hidden',border:`1px solid ${T.border}`,background:T.card}}><InlineTVChart symbol={sel2.tv}/></div>
        </div>
      )}
      {layout==='2h'&&(
        <div style={{display:'flex',gap:5,overflowX:'auto',marginBottom:10}}>
          {TV_FEATURED.slice(0,10).map(f=>(
            <button key={f.tv} onClick={()=>setSel2(f)} style={{flexShrink:0,background:sel2.tv===f.tv?f.clr+'20':T.card,border:`1px solid ${sel2.tv===f.tv?f.clr:T.border}`,borderRadius:8,padding:'4px 9px',cursor:'pointer'}}>
              <div style={{color:sel2.tv===f.tv?f.clr:T.txt,fontSize:9,fontWeight:700}}>{f.sym}</div>
            </button>
          ))}
        </div>
      )}
      ...
      <div style={{display:'flex',gap:6,marginBottom:8}}>
        <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="검색 (BTC, AAPL, 005930…)"
          style={{flex:1,background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:'9px 12px',color:T.txt,fontSize:12,outline:'none'}}/>
        <button onClick={()=>setManualMode(v=>!v)} style={{background:manualMode?T.acg:'transparent',color:manualMode?T.acl:T.muted,border:`1px solid ${manualMode?T.acl:T.border}`,borderRadius:10,padding:'9px 12px',fontSize:11,fontWeight:700,cursor:'pointer'}}>⌨️ 직접</button>
      </div>
      ...
      <div style={{display:'flex',gap:4,overflowX:'auto',paddingBottom:3,marginBottom:8}}>
        {['all','crypto','stock','krstock','etf','index','commodity','forex'].map(c=>(
          <button key={c} onClick={()=>setCatFilt(c)} style={{flexShrink:0,padding:'3px 9px',background:catFilt===c?T.acg:'transparent',color:catFilt===c?T.acl:T.muted,border:`1px solid ${catFilt===c?T.acl:T.border}`,borderRadius:20,fontSize:10,fontWeight:700,cursor:'pointer'}}>
            {c==='all'?'전체':c==='crypto'?'코인':c==='stock'?'미국주식':c==='krstock'?'한국주식':c==='etf'?'ETF':c==='index'?'지수':c==='commodity'?'원자재':'환율'}
          </button>
        ))}
      </div>

      {/* Asset grid with live prices */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:5,marginBottom:12}}>
        {filtered.map(f=>{
          const base=f.sym.replace('USDT','');
          const live=liveData[base]||liveData[f.sym];
          return (
            <button key={f.tv} onClick={()=>{setSel(f);setQuery('');}}
              style={{background:sel.tv===f.tv?f.clr+'20':T.card,border:`2px solid ${sel.tv===f.tv?f.clr:T.border}`,borderRadius:10,padding:'8px 5px',cursor:'pointer',textAlign:'center'}}>
              <div style={{width:22,height:22,borderRadius:6,background:`${f.clr}20`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:8,fontWeight:900,color:f.clr,margin:'0 auto 2px'}}>{f.sym.slice(0,3)}</div>
              <div style={{color:sel.tv===f.tv?f.clr:T.txt,fontWeight:700,fontSize:9,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{f.sym}</div>
              {live?(
      ...
      {provStatus&&(
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:'10px 14px',marginBottom:8}}>
          <div style={{color:T.muted,fontSize:10,fontWeight:700,marginBottom:5}}>📡 데이터 소스</div>
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            {[{n:'Binance',s:'live'},{n:'CoinGecko',s:'live'},{n:'Polygon',s:'unconfigured'},{n:'Mock',s:provStatus.status==='mock'?'active':'standby'}].map(p=>(
              <div key={p.n} style={{display:'flex',gap:3,alignItems:'center'}}>
                <div style={{width:6,height:6,borderRadius:'50%',background:p.s==='live'||p.s==='active'?T.grn:T.muted}}/>
                <span style={{color:T.muted,fontSize:9}}>{p.n}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{textAlign:'center'}}>
        <a href="/chart" target="_blank" style={{color:T.acl,fontSize:11,fontWeight:700,textDecoration:'none'}}>📊 전용 차트 페이지에서 4분할 + 전체화면으로 보기 ↗</a>
      </div>
    </div>
  );
}
...
const MTABS=[
  ...
  {id:'hedgeos',label:'Hedge OS',icon:'🏦'},{id:'intelligence',label:'인텔리전스',icon:'🧠'},{id:'chart',label:'차트',icon:'📈'},{id:'wunder',label:'WUNDER봇',icon:'🤖'},{id:'tradfi',label:'TradFi',icon:'📊'},{id:'realtime',label:'실시간',icon:'📡'},
