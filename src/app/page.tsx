...
function WatchlistPage({prices,currency,onNav}:{prices:Asset[];currency:string;onNav:(tab:string)=>void}) {
  ...
  /* ─── State ─── */
  const [wl,    setWl]     = useState<any[]>(()=>{ try{return JSON.parse(localStorage.getItem(STORE)||'[]');}catch{return [];} });
...
export default function App() {
  ...
  const renderPage=useCallback(()=>{
    ...
    const map:Record<string,React.ReactNode>={
      ...
      watchlist:<WatchlistPage prices={prices} currency={currency} onNav={nav}/>,
...
// Legacy LOGO_SOURCES for backward compat
const LOGO_SOURCES: Record<string, string> = Object.fromEntries(
  Object.entries(LOGO_DB)
    .filter(([,d]) => d.primary)
    .map(([k,d]) => [k, d.primary])
);