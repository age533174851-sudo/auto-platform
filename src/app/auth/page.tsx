...
export default function AuthPage() {
  ...
  // Supabase 실제 도달 여부 (env 존재만 보는 false-positive 방지)
  const [health, setHealth] = useState<{ ok:boolean; reason:string; status?:number; url:string }|null>(null);
  useEffect(()=>{ if (SUPABASE_CONFIGURED) sbHealthCheck().then(setHealth).catch(()=>setHealth({ ok:false, reason:'unreachable', url:SUPABASE_URL })); }, []);
      ...
      <div style={{width:'100%',maxWidth:420}}>
        ...
        <div style={{textAlign:'center',marginBottom:24}}>
          ...
          {(()=>{
            ...
            const reasonMsg: Record<string,string> = { env_missing:'환경변수 누락', invalid_url:'URL 형식 오류(값에 공백/오타 가능)', unreachable:'서버 도달 불가(프로젝트 일시정지 의심)', timeout:'응답 시간 초과', ok:'연결됨' };
            const c = checking ? T.muted : ok ? T.grn : T.red;
            let host = ''; try { host = new URL(SUPABASE_URL).host; } catch { host = SUPABASE_URL || '(빈 값)'; }
            return (
              <div style={{marginTop:8}}>
                <div style={{display:'inline-block',padding:'3px 10px',borderRadius:99,background:c+'15',border:`1px solid ${c}30`}}>
                  <span style={{fontSize:10,fontWeight:700,color:c}}>
                    {checking ? '⏳ Supabase 확인 중…' : ok ? '✅ Supabase 연결됨' : `⚠️ Supabase ${reasonMsg[health!.reason]||health!.reason}`}
                  </span>
                </div>
                {!checking && !ok && (
                  <div style={{marginTop:6,fontSize:9,color:T.muted,lineHeight:1.5,maxWidth:300,margin:'6px auto 0'}}>
                    접속 대상: <span style={{color:T.txt}}>{host}</span>{health?.status?` · HTTP ${health.status}`:''}<br/>
                    환경변수 존재는 하지만 실제 접속이 안 됩니다. Supabase 대시보드에서 프로젝트가 <b>Paused</b>인지, Vercel의 <b>NEXT_PUBLIC_SUPABASE_URL</b> 값에 공백/줄바꿈이 없는지 확인 후 <b>재배포</b>하세요.
                  </div>
                )}
              </div>
            );
