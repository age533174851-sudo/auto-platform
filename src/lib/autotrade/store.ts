...
export function reversePaperPosition(asset: string, currentPrice: number): { ok: boolean; pnl: number; closedValue: number } {
  const b = loadPaperBalance();
  const pos = b.positions[asset];
  if (!pos || pos.qty <= 0) return { ok: false, pnl: 0, closedValue: 0 };
  const closedValue = pos.qty * currentPrice;
  const res = closePaperPosition(asset, currentPrice, 1);
  return { ok: res.ok, pnl: res.pnl, closedValue };
}
