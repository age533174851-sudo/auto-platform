...
export function runOrderCurrencyTests() {
  ...
  test('연습 잔고가 거래소 비율의 근거가 되지 않는다', () => {
    const r = percentBaseFor({ mode: 'testnet', practiceKrw: 10_000_000, availableUsdt: 250 });
    eq(r.currency, 'USDT');
    eq(r.base, 250);                      // 원화 천만이 아니라 가용 USDT
  });

  test('모의는 연습 원화 잔고를 쓴다', () => {
    const r = percentBaseFor({ mode: 'mock', practiceKrw: 10_000_000, availableUsdt: 250 });
    eq(r.currency, 'KRW');
    eq(r.base, 10_000_000);
  });

  test('가용 USDT를 못 읽으면 null이다 — 연습 잔고로 대신하지 않는다', () => {
    for (const bad of [null, undefined, 0, NaN, -5]) {
      const r = percentBaseFor({ mode: 'live', practiceKrw: 10_000_000, availableUsdt: bad as any });
      eq(r.base, null);
    }
  });
}
