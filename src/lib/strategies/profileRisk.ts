...
function fresh(id: StrategyType, cycleNo = 1, cycles: CycleRecord[] = []): ProfileRiskState {
 ...
 */
export function recordProfileTrade(id: StrategyType, pnl: number, holdSec = 0): ProfileRiskState {
  ...
  // 2) 손익 반영
  s.realizedPnL += pnl;
  s.equity = seedOf(id) + s.realizedPnL;
 ...
 */
export function completeCycle(id: StrategyType, reason: string, reached: boolean): ProfileRiskState {
  const s = loadProfileRisk(id);
...
export function resetProfileRisk(id: StrategyType): ProfileRiskState {
  ...
  const s = fresh(id, cur.cycleNo, cur.cycles);
  save(s);
  return s;
}
