...
export const STRATEGIES: StrategySpec[] = [
  ...
  },
  {
    ...
    checkFlag: 'dryRun',
    // **코드가 발전했는데 이 문장이 남아 있었다.**
    //
    // 예전에는 "진입 방향과 손절·익절 규칙이 아직 입력되지 않았습니다 —
    // 주문은 나가지 않습니다"였다. 그런데 진입 방향(09:10~09:30 합성 봉)과
    // 청산 정책(testnet-exit-v1)이 확정됐고 Gate TESTNET에서 실주문·
    // 보호주문·자동청산까지 확인됐다. 화면은 이 문장을 그대로 보여준다 —
    // 사용자는 켜도 안 나간다고 읽는다.
    note: 'KST 09:10~09:30 합성 봉으로 방향을 정하고 하루 한 번만 진입합니다. '
      + '청산은 별도 정책(testnet-exit-v1)이며 원본 규칙이 아닙니다. '
      + '실전은 테스트넷 검증이 끝날 때까지 닫아 둡니다',
  },
];
