...
export interface StrategyRisk {
  ...
  leverage?:        number;     // 선물 전략 레버리지 (수동 지정 시)
  // 고정 리스크 모델 (Fixed Fractional Sizing)
  useFixedRisk?:    boolean;    // true면 리스크% 기반 수량 자동 계산
  riskPerTradePct?: number;     // 1회 매매 최대 손실 (총자본 대비 %, 예: 2)
  atrMultiplier?:   number;     // ATR 손절 배수 (예: 1.5)
  maxLeverage?:     number;     // 역산 레버리지 상한 (안전, 예: 10)
}
...
export interface UserStrategy {
  ...
  avoidEconEvents?: boolean;   // CPI/FOMC/NFP 발표 전후 거래중지
  source?:     'manual' | 'ai';
