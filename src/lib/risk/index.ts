...
export function calcPositionSize(input: SizingInput): SizingResult {
  ...
  // 상한: 명목가치는 자본×레버리지 이내 (마진이 자본을 못 넘게)
  // 추가로 단일 포지션 명목가치를 자본의 1배(현물 기준) ~ 레버리지배 이내로 제한
  const maxNotional = capital * Math.max(1, Math.min(leverage, 10));
  const cappedPosition = Math.max(0, Math.min(positionSize, maxNotional));

  const quantity       = cappedPosition / entryPrice;
  const marginRequired = cappedPosition / Math.max(1, leverage);
