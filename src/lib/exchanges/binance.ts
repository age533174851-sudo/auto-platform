...
export function binanceBase(opts?: { testnet?: boolean; futures?: boolean }): string {
  ...
  return BASE;   // spot은 testnet 분기 생략 (현물 testnet은 별도)
}
...
export async function getSpotSymbolFilters(symbol: string): Promise<{ stepSize: number; minQty: number } | null> {
  const sym = symbol.toUpperCase().replace('/', '');
  const cached = _spotLotCache[sym];
  if (cached && Date.now() - cached.at < 5 * 60 * 1000) return cached;
  try {
    const r = await fetch(`${BASE}/api/v3/exchangeInfo?symbol=${sym}`, { signal: AbortSignal.timeout(8000) });
    if (!r.ok) return null;
