...
export function validateAnalysis(
  raw: any,
  ctx: { sourceUrl?: string; publishedAt?: string; provider: string; model: string; now?: string },
): ValidateResult {
  const errors: string[] = [];
  ...
  const sourceUrl = String(ctx.sourceUrl ?? '').trim();
  if (!sourceUrl) errors.push('sourceUrl 없음 (원문 없는 분석은 저장하지 않는다)');
  if (raw.sourceUrl && String(raw.sourceUrl).trim() !== sourceUrl) {
    repaired.push('모델이 만든 sourceUrl을 원문 값으로 되돌림');
  }

  const publishedAt = String(ctx.publishedAt ?? '').trim();
  if (!publishedAt) errors.push('publishedAt 없음');
