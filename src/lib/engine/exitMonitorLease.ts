...
// ── 임차를 실제로 잡는 순서 ──────────────────────────
//
// 위 `leaseDecision`은 **판정**만 한다. 그 판정을 표에 쓰는 순서가 따로
// 있고, 거기에 실제 경합 구멍이 있었다:
//
//   A: SELECT fence=7 → "다음은 8" → UPSERT fence=8  ✅
//   B: SELECT fence=7 → "다음은 8" → UPSERT fence=8  ✅  ← 둘 다 주인
//
// 뒤의 `fenceStillMine()`도 둘 다 8을 보므로 통과한다. 즉 울타리가 두
// 실행을 구분하지 못한다 — 같은 포지션에 손절 이동·청산이 두 번 나갈 수
// 있고, 그건 되돌릴 수 없다.
//
// 그래서 **본 값을 조건으로 걸어** 쓴다(compare-and-set). 그 사이 누가
// 끼어들었으면 갱신되는 줄이 0개이고, 그때는 물러난다.
//
// 이 순서가 라우트 밖에 있는 이유는 하나다: **경합을 시험으로 재현할 수
// 있어야 한다.** 라우트 안에서는 두 요청을 겹쳐 돌려 볼 수 없다.
 ...
 *
 * 못 잡으면 `granted: false`이고, 그때 거래소 쓰기는 **0이어야 한다.**
 */
export async function acquireExitLease(i: {
  ...
  try {
    ...
    if (got?.missingTable) {
      ...
      // 그건 이 안전장치가 막으려던 것보다 나쁘다. 예전 의미를 유지한다.
  ...
  // **못 읽은 것을 빈 표로 읽지 않는다.** `leaseDecision`이 UNKNOWN으로
  // 막는다 — 그 의미를 여기서 다시 쓰지 않는다.
  ...
  try {
    if (prevFence == null) {
      // 줄이 없었다 → 만들면서 잡는다. 동시에 둘이 만들면 하나만 성공한다.
