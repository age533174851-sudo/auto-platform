...
export function lifecycleDecide(i: {
  ...
  //
  // ★ **정책을 먼저 본다.** 값이 비어 있다는 사실과 "고정 손절을 쓰지
  //   않기로 했다"는 사실은 다르다. 앞의 것만 보면, 손절을 걸다 실패한
  //   포지션까지 무손절 계약처럼 다루게 된다.
  if (p.stopPolicy === 'NO_FIXED_SL') {
    return no('NO_FIXED_STOP',
      '이 포지션은 고정 손절을 쓰지 않는 계약입니다 — 없는 1R을 지어내지 않고, '
      + '손절을 새로 걸지도 않습니다 (시간 청산만 적용됩니다)');
  }
