...
export function runLifecycleActionTests() {
  ...
  test('★ 정상: 보내고 포지션 0을 확인하면 ok', async () => {
  ...
  test('★ 임차를 잃으면 청산을 **보내지 않는다**', async () => {
  ...
  test('★ 권한 확인이 던져도 보내지 않는다 (fail-closed)', async () => {
    ...
    const r = await applyLifecycleClose({
      stillMine: async () => { calls.push('m'); throw new Error('DB 끊김'); },
  ...
  test('★ 종료 후 재조회 실패를 "닫혔다"로 읽지 않는다', async () => {
  ...
  test('거래소가 거부하면 재조회하지 않고 거부로 적는다', async () => {
