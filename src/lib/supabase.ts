...
// env 값에 섞이는 공백/줄바꿈/끝슬래시 제거 (Vercel 흔한 함정 — 이게 있으면 "Failed to fetch")
const RAW_URL = process.env.NEXT_PUBLIC_SUPABASE_URL  || '';
const RAW_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '';
export const SUPABASE_URL  = RAW_URL.trim().replace(/\/+$/, '');
export const SUPABASE_KEY  = RAW_KEY.trim();
export const SUPABASE_CONFIGURED = !!(SUPABASE_URL && SUPABASE_KEY);

// URL 형식이 유효한지 (https://xxx.supabase.co 모양)
export function isSupabaseUrlValid(): boolean {
  if (!SUPABASE_URL) return false;
  try { return new URL(SUPABASE_URL).protocol === 'https:'; }
  catch { return false; }
}

// 실제 Supabase 도달 가능 여부 (배지/진단용 — env 존재만 보는 false positive 방지)
export async function sbHealthCheck(): Promise<{ ok: boolean; reason: string; status?: number; url: string }> {
  if (!SUPABASE_CONFIGURED) return { ok: false, reason: 'env_missing', url: SUPABASE_URL };
  try { new URL(SUPABASE_URL); } catch { return { ok: false, reason: 'invalid_url', url: SUPABASE_URL }; }
  try {
    const r = await fetch(`${SUPABASE_URL}/auth/v1/health`, { headers: { apikey: SUPABASE_KEY }, signal: AbortSignal.timeout(6000) });
    if (r.ok) return { ok: true, reason: 'ok', status: r.status, url: SUPABASE_URL };
    return { ok: false, reason: `http_${r.status}`, status: r.status, url: SUPABASE_URL };
  } catch (e: any) {
    // 여기 도달 = 네트워크 실패 (프로젝트 일시정지 / URL 오타 / 도달 불가)
    return { ok: false, reason: e?.name === 'TimeoutError' ? 'timeout' : 'unreachable', url: SUPABASE_URL };
  }
}
