...
export async function validatePreOrder(
  ...
  const maxDailyLoss = limits.maxDailyLossKRW;
  if (todayLoss >= maxDailyLoss) {
    ...
    blockers.push(`일일 손실 한도(₩${maxDailyLoss.toLocaleString()}) 초과 — 오늘 거래 중단`);
    allowed = false;
  } else {
    const ratio = todayLoss / maxDailyLoss;
    if (ratio > 0.7) warnings.push(`⚠️ 일일 손실 ${Math.round(ratio * 100)}% 소진 (₩${todayLoss.toLocaleString()} / ₩${maxDailyLoss.toLocaleString()})`);
  ...
  return {
    ...
    maxLoss:         Math.round(maxPossibleLoss),
    requiresConfirm,
  };
}
