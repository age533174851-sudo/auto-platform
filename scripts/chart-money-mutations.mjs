...
const MUTATIONS = [
  ...
  // ══ 주문 시트가 차트를 가린다 ══
  { name: '시트가 화면을 다 덮는다',
    file: 'src/lib/trading/sheetSnap.ts',
    cut: ["  return snap === 'EXPANDED' ? 88 : 52;", "  return snap === 'EXPANDED' ? 100 : 100;"] },
  { name: '시트를 확장 자리에서 연다 — 차트가 가려진 채로 시작한다',
    file: 'src/lib/trading/sheetSnap.ts',
    cut: ["export const DEFAULT_SNAP: SheetSnap = 'HALF';",
          "export const DEFAULT_SNAP: SheetSnap = 'EXPANDED';"] },
  { name: 'HALF에서 주문 버튼을 뺀다 — 주문하려면 먼저 펴야 한다',
    file: 'src/lib/trading/sheetSnap.ts',
    cut: ["const IN_HALF: SheetSection[] = ['BOOK', 'SIDE', 'SIZING', 'SUBMIT'];",
          "const IN_HALF: SheetSection[] = ['BOOK', 'SIDE', 'SIZING'];"] },
