...
const FILES = [
  ...
  'src/components/instrument/InstrumentDetail.tsx',
  'src/components/pages/MarketPage.tsx',
  'src/app/page.tsx',
];
...
const MUTATIONS = [
  ...
  // ══ 거래 화면이 남의 차트로 돌아간다 ══
  { name: '종목 상세 기본 차트를 iframe으로 되돌린다',
    file: 'src/components/instrument/InstrumentDetail.tsx',
  ...
  // ══ 새 상세 화면이 지어낸 값을 물려받는다 ══
  { name: "상세가 지어낸 기사 경로를 읽는다",
    file: "src/components/instrument/InstrumentDetail.tsx",
    cut: ["'/api/news/stored?limit=30'",
          "'/api/market/news?limit=30'"] },
  { name: "상세가 손으로 적은 펀더멘털을 끌어온다",
    file: "src/components/instrument/InstrumentDetail.tsx",
    cut: ["  const plan = instrumentFieldPlan(market);",
          "  const plan = instrumentFieldPlan(market); const roe = 11.2;"] },
  { name: "상세가 보유를 기본 계좌 라우트에서 읽는다",
    file: "src/components/instrument/InstrumentDetail.tsx",
    cut: ["  const ledger = usePaperLedger(target, !!auth);",
          "  const ledger = usePaperLedger(target, !!auth); void fetch('/api/paper/account');"] },
  { name: "상세가 칸 권위를 묻지 않고 그린다",
    file: "src/components/instrument/InstrumentDetail.tsx",
    cut: ["  const plan = instrumentFieldPlan(market);",
          "  const plan = { kind:'CRYPTO', marketCap:{availability:'AVAILABLE',reason:null}, dividend:{availability:'AVAILABLE',reason:null}, companyInfo:{availability:'AVAILABLE',reason:null}, news:{availability:'AVAILABLE',reason:null}, chart:{availability:'AVAILABLE',reason:null} } as any;"] },
  { name: "상세가 변동률 기준을 스스로 정한다",
    file: "src/components/instrument/InstrumentDetail.tsx",
    cut: ["  const change = changeView({ market, price: stream.lastPrice, changePct: stream.changePct });",
          "  const change = { basis:'ROLLING_24H', label:'전일대비', pct: stream.changePct, amount:null, unknownReason:null } as any;"] },
  { name: "옛 모달을 다시 정본으로 되돌린다",
    file: "src/app/page.tsx",
    cut: ["      {detailTarget ? (",
          "      {false ? ("] },
  { name: "목록 기본 정렬을 주당 가격순으로 되돌린다",
    file: "src/components/pages/MarketPage.tsx",
    cut: ["  const [sort,       setSort]       = useState('tradingValue');",
          "  const [sort,       setSort]       = useState('price');"] },
  { name: "거래대금 순위에서 통화 구분을 없앤다",
    file: "src/lib/markets/ranking.ts",
    cut: ["    const bucket = byCurrency.get(tv.currency);",
          "    const bucket = byCurrency.get('ALL');"] },
  { name: "시총순을 고를 수 있게 연다",
    file: "src/lib/markets/ranking.ts",
    cut: ["export const MARKET_CAP_RANKING_AVAILABLE = false;",
          "export const MARKET_CAP_RANKING_AVAILABLE = true;"] },
  { name: "코인에 배당 칸을 만든다",
    file: "src/lib/markets/instrumentFields.ts",
    cut: ["    return { availability: 'HIDDEN', reason: '암호화폐에는 배당 개념이 없습니다' };",
          "    return { availability: 'UNAVAILABLE', reason: '암호화폐에는 배당 개념이 없습니다' };"] },
