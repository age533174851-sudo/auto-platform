...
import { variableFunctionBody } from './lib/function-body.mjs';
...
const stopFn = variableFunctionBody(raw[AUTO], 'handleGlobalStop');
if (!stopFn.ok) {
  ...
  if (!/summarize\s*\(/.test(stopBody) || !/unknownResult\s*\(/.test(stopBody)) {
    err(`${AUTO}: 전체정지 결과를 globalStop의 판정(summarize/unknownResult)으로 만들지 않습니다 — 화면이 스스로 결론을 적으면 그 결론은 서버와 갈립니다`);
  }
  ...
  const ALLOWED_RESULT = /^(summarize\s*\(|unknownResult\s*\(|IDLE_RESULT|\{\s*\.\.\.\s*IDLE_RESULT)/;
  ...
  for (const m of sites) {
    if (!ALLOWED_RESULT.test(m[1].trim())) {
      err(`${AUTO}: 전체정지 결과를 화면이 직접 지어냅니다 — \`${m[1].trim().slice(0, 30)}…\`. `
        + 'summarize()/unknownResult()가 센 것만 넣으세요');
    }
  }

  /* 화면이 직접 쓸 수 있는 상태는 '아직 모른다'(STOPPING)뿐이다.
     ALL_STOPPED·PARTIAL은 서버 응답을 센 결과여야 한다. */
