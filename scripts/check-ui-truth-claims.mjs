...
/* ── 주기를 정확한 숫자로 약속하지 않는다 ────────────────────
   AutoTradeEngine의 setInterval은 60초가 맞다. 하지만 각 전략이 60초마다
   평가되는 것은 아니다:
     · 앞 tick이 안 끝났으면 runningRef 때문에 그 tick을 건너뛴다
     · 신호가 난 전략은 RE_EVAL_COOLDOWN_MS(5분)만큼 재평가를 미룬다
     · 브라우저 엔진이므로 앱을 닫으면 아예 안 돈다
   "60초마다 평가합니다"는 폴링 주기를 전략 평가 주기로 바꿔 말한 것이고,
   사용자는 그 숫자로 "1분이면 반응한다"를 기대한다. */
for (const f of [AUTO, RAIL]) {
  deny(f, /\d+\s*(초|분)마다[^.\n]{0,24}(평가|판단|검사|확인)/,
    '전략 평가 주기를 정확한 초로 약속합니다 — 폴링 주기와 평가 주기는 다릅니다(tick 건너뜀·5분 쿨다운)');
}

/* ── 배선되지 않은 탭이 실제 신호를 보여 준다고 말하지 않는다 ──
   `const [signals] = useState(INITIAL_SIGNALS)` — setter도, 서버/로그를
   읽는 경로도 없다. 엔진이 신호를 내도 이 탭으로 들어오지 않는다.
   그 사실을 화면이 직접 말해야 한다. */
if (!/setSignals/.test(code[AUTO])) {
  need(AUTO, /신호 탭에는[\s\S]{0,60}연결되어 있지 않습니다/,
    '신호 탭이 실제 신호에 배선돼 있지 않은데 그 사실을 말하지 않습니다');
  deny(AUTO, /실제 신호(는|가)[^.\n]{0,40}(여기|이 (탭|화면))에[^.\n]{0,20}(표시|나옵니다|들어옵니다)/,
    '배선되지 않은 탭이 실제 신호를 보여 준다고 말합니다');
}
